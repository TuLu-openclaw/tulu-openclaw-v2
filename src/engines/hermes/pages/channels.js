...
} from '../lib/channel-config-policy.js'
...
const CHANNELS = [
  ...
  },
  {
    ...
    secretFields: ['serverPassword', 'nickservPassword'],
    fields: [
...
const RUNTIME_STATUS_KEYS = {
  connected: 'engine.hermesChannelRuntimeConnected',
  connecting: 'engine.hermesChannelRuntimeConnecting',
  failed: 'engine.hermesChannelRuntimeFailed',
  paused: 'engine.hermesChannelRuntimePaused',
  disabled: 'engine.hermesChannelRuntimeDisabled',
  disconnected: 'engine.hermesChannelRuntimeDisconnected',
  stopped: 'engine.hermesChannelRuntimeStopped',
  stale: 'engine.hermesChannelRuntimeStale',
  unknown: 'engine.hermesChannelRuntimeUnknown',
}

function runtimeLabel(status) {
  return t(RUNTIME_STATUS_KEYS[status] || RUNTIME_STATUS_KEYS.unknown)
}
...
function renderField(field, form, disabled) {
  const rawValue = valueOf(form, field.key)
  const masked = field.type === 'password' && Boolean(rawValue.trim())
  const value = masked ? '' : rawValue
  ...
  return `
    <label class="hm-field">
      <span class="hm-field-label">${label}${masked ? `<span class="hm-channel-secret-state">${esc(t('engine.hermesChannelSecretConfigured'))}</span>` : ''}</span>
      <input class="hm-input hm-channel-input" data-key="${esc(field.key)}" type="${esc(field.type || 'text')}" value="${esc(value)}" ${masked ? 'data-secret-configured="true"' : ''} ${disabled ? 'disabled' : ''} placeholder="${esc(placeholder)}" autocomplete="off">
    </label>
  `
}

function collectForm(el, platform, previous = {}) {
  ...
  el.querySelectorAll('.hm-channel-input').forEach(input => {
    ...
    else if (input.type === 'password' && input.dataset.secretConfigured === 'true' && !input.value.trim()) form[key] = previous[key] || '***'
...
export function render() {
  ...
  let configRevision = ''
  let envRevision = ''
  let runtime = normalizeHermesChannelRuntime({})
  let loading = true
  let saving = false
  let error = ''
  let success = ''
  const dirtyPlatforms = new Set()
  ...
  function draw() {
    ...
    const connectedCount = CHANNELS.filter(item => runtime.platforms[item.id]?.status === 'connected').length
    const activeRuntime = runtime.platforms[active] || { status: 'unknown' }
    ...
    el.innerHTML = `
      ...
      <section class="hm-channel-summary" aria-label="${esc(t('engine.hermesChannelSummary'))}">
        ...
        <div class="hm-channel-stat"><span>${esc(t('engine.hermesChannelRuntimeConnectedCount'))}</span><strong>${connectedCount}</strong></div>
        <div class="hm-channel-stat"><span>${esc(t('engine.hermesChannelGatewayState'))}</span><strong>${esc(runtime.gatewayState)}</strong></div>
      </section>
      ...
      <div class="hm-channel-layout">
          ...
          <div class="hm-panel-body hm-panel-body--tight">
            <div class="hm-channel-list" role="tablist" aria-label="${esc(t('engine.hermesChannelPlatforms'))}">
              ${CHANNELS.map(item => {
                ...
                const itemRuntime = runtime.platforms[item.id] || { status: 'unknown' }
                return `
                  <button class="hm-channel-tab ${item.id === active ? 'is-active' : ''}" data-channel="${esc(item.id)}" role="tab" aria-selected="${item.id === active ? 'true' : 'false'}" ${disabled ? 'disabled' : ''}>
                    ...
                    <span class="hm-channel-tab-main">
                      ...
                      <small>${esc(itemForm.enabled ? runtimeLabel(itemRuntime.status) : t('engine.hermesChannelDisabled'))}</small>
                    </span>
                    <span class="hm-channel-dot is-${esc(itemRuntime.status)}" aria-hidden="true"></span>
                  </button>
                `
              }).join('')}
            </div>
        ...
        <section class="hm-panel hm-channel-form-panel">
          ...
          <div class="hm-panel-body">
            ...
            ` : `
              <div class="hm-channel-runtime-card is-${esc(activeRuntime.status)}">
                <div>
                  <span>${esc(t('engine.hermesChannelRuntimeStatus'))}</span>
                  <strong>${esc(runtimeLabel(activeRuntime.status))}</strong>
                </div>
                ${activeRuntime.upstreamState ? `<code>${esc(activeRuntime.upstreamState)}</code>` : ''}
                ${(activeRuntime.errorCode || activeRuntime.errorMessage) ? `<p>${esc([activeRuntime.errorCode, activeRuntime.errorMessage].filter(Boolean).join(': '))}</p>` : ''}
              </div>
    ...
    el.querySelectorAll('.hm-channel-input').forEach(input => {
      input.addEventListener(input.type === 'text' || input.tagName === 'TEXTAREA' ? 'input' : 'change', () => {
        dirtyPlatforms.add(active)
      })
    })
    el.querySelectorAll('.hm-channel-tab').forEach(button => {
      button.addEventListener('click', () => {
        if (!loading && !saving) values = { ...values, [active]: collectForm(el, active, values[active]) }
  ...
  async function load() {
    ...
    try {
      ...
      values = parseHermesChannelConfig(sourceYaml, data?.envValues || {})
      runtime = normalizeHermesChannelRuntime(data?.runtime || {})
      configPath = data?.configPath || ''
      configRevision = data?.configRevision || ''
      envRevision = data?.envRevision || ''
    } catch (err) {
      error = errorText(err, t('engine.hermesChannelLoadFailed'))
    } finally {
      loading = false
      draw()
    }
  ...
  async function save() {
    const form = collectForm(el, active, values[active])
    ...
    draw()
    try {
      let mergedYaml = sourceYaml
      const envUpdates = {}
      const savePlatforms = hermesChannelSavePlatforms(sourceYaml, dirtyPlatforms)
      for (const platform of savePlatforms) {
        const draft = normalizeForm(platform, values[platform])
        mergedYaml = mergeHermesChannelConfig(mergedYaml, platform, draft)
        const envDraft = { ...draft }
        for (const secretKey of channelMeta(platform).secretFields) {
          if (isMaskedSecret(envDraft[secretKey])) envDraft[secretKey] = ''
        }
        Object.assign(envUpdates, buildHermesChannelEnvUpdates(platform, envDraft))
      }
      const result = await api.hermesChannelConfigSave(
        mergedYaml,
        envUpdates,
        configRevision,
        envRevision,
      )
      sourceYaml = result?.yaml || mergedYaml
      configRevision = result?.configRevision || configRevision
      envRevision = result?.envRevision || envRevision
      dirtyPlatforms.clear()
      success = t('engine.hermesChannelSaved')
      toast(success, 'success')
    } catch (err) {
      error = errorText(err, t('engine.hermesChannelSaveFailed'))
      toast(error, 'error')
    } finally {
      saving = false
      draw()

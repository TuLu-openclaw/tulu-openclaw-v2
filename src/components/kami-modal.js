...
import { login, revalidate, getStoredKami, saveKami, markVerified, getNotice, KAMI_CONFIG } from '../lib/kami.js'
...
function showBlockOverlay() {
  ...
  overlay.innerHTML =
    '<div style="text-align:center;max-width:400px;padding:40px">' +
      ...
      '<div style="font-size:22px;font-weight:700;color:#ef4444;margin-bottom:12px">' + t('kami.authFailedTitle') + '</div>' +
      '<div style="font-size:15px;color:#999;margin-bottom:8px">' + t(KAMI_CONFIG.errorMessageKey || 'kami.unauthorizedMessage') + '</div>' +
      '<div style="font-size:12px;color:#555;margin-bottom:32px">' + t('kami.authExpired') + '</div>' +
      '<button id="kami-block-retry" style="padding:12px 36px;font-size:15px;font-weight:600;color:#fff;' +
        ...
        'cursor:pointer;box-shadow:0 4px 14px rgba(99,102,241,0.4)">' + t('kami.retryVerify') + '</button>' +
    '</div>'
...
function buildModalHTML(initialKami, showPw, remembered, noticeText) {
  ...
  var toggleTitle = showPw ? t('kami.hidePassword') : t('kami.showPassword')
  ...
  var noticeHTML =
    '<div id="kami-announcement" style="' +
      ...
      '<span id="kami-notice-text">' + (noticeText || t('kami.defaultNotice')) + '</span>' +
    '</div>'
    ...
    '<div style="' +
      ...
      '<div style="margin-bottom:16px">' +
        ...
        '<div style="position:relative">' +
          '<input id="kami-input" type="' + pwType + '" placeholder="' + _escapeAttr(t('kami.licensePlaceholder')) + '" autocomplete="off"' + inputVal +
      ...
      '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">' +
        ...
        '<a href="https://qm.qq.com/q/FF8D891UWc" target="_blank" rel="noopener"' +
          ' style="font-size:11px;color:#6366f1;text-decoration:none">' + t('kami.buyKey') + ' &#x2192;</a>' +
      '</div>' +

      '<button id="kami-verify-btn" style="' +
        'width:100%;padding:13px;font-size:15px;font-weight:700;color:#fff;' +
        'background:linear-gradient(135deg,#6366f1,#8b5cf6);border:none;border-radius:10px;' +
        'cursor:pointer;transition:opacity 0.2s;box-shadow:0 4px 14px rgba(99,102,241,0.35)">' + t('kami.verifyButton') + '</button>' +

      '<div id="kami-error" style="margin-top:12px;text-align:center;font-size:12px;color:#ef4444;min-height:16px"></div>' +

      '<div style="margin-top:20px;text-align:center">' +
        '<a href="https://qm.qq.com/q/FF8D891UWc" target="_blank" rel="noopener"' +
          ' style="font-size:11px;color:#555;text-decoration:none">' + t('kami.officialSystem') + '</a>' +
      '</div>' +
    '</div>'
}
...
async function showKamiModal(isRetry) {
  ...
  toggleBtn.addEventListener('click', function() {
    ...
    toggleBtn.title = isPasswordVisible ? t('kami.hidePassword') : t('kami.showPassword')
  })
  ...
  async function doVerify() {
    ...
    if (!kami) {
      errorEl.textContent = t('kami.emptyKey')
      return
    }
    ...
    verifyBtn.innerHTML = '<span style="display:inline-block;width:16px;height:16px;border:2px solid rgba(255,255,255,0.3);border-top-color:#fff;border-radius:50%;animation:spin 0.7s linear infinite;vertical-align:middle;margin-right:8px"></span>' + t('kami.verifying')
      ...
      if (card) {
        card.innerHTML =
          '<div style="text-align:center;padding:40px 0">' +
            ...
            '<div style="font-size:18px;font-weight:700;color:#22c55e;margin-bottom:10px">' + t('kami.successTitle') + '</div>' +
            '<div style="font-size:13px;color:#888">' + t('kami.enteringApp') + '</div>' +
          '</div>'
      }

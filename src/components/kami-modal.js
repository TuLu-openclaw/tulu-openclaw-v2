...
function showBlockOverlay() {
  ...
  overlay.innerHTML =
    '<div style="text-align:center;max-width:400px;padding:40px">' +
      '<div style="font-size:64px;margin-bottom:20px">[LOCK]</div>' +
  ...
  document.getElementById('kami-block-retry').addEventListener('click', () => {
    overlay.remove()
    showKamiModal()
  })
}
...
//  构建弹框 HTML
// =============================================
function buildModalHTML(initialKami, showPw, remembered) {
  const pwType = showPw ? 'text' : 'password'
  const toggleIcon = showPw ? '[HIDE]' : '[SHOW]'
  const toggleTitle = showPw ? '隐藏密码' : '显示密码'
  const checkedAttr = remembered ? 'checked' : ''
  const inputVal = initialKami ? ' value="' + initialKami + '"' : ''
  const spinStyle =
  ...
  return '<style>@keyframes spin{to{transform:rotate(360deg)}}</style>' +
    '<div style="background:#1a1a2e;border-radius:16px;padding:36px;width:380px;max-width:90vw;' +
      'box-shadow:0 24px 80px rgba(0,0,0,0.6);border:1px solid rgba(255,255,255,0.08)">' +
      '<div style="text-align:center;margin-bottom:28px">' +
        '<div style="font-size:48px;margin-bottom:12px">[KEY]</div>' +
      ...
      '<div style="margin-bottom:16px">' +
        ...
        '<div style="position:relative">' +
          '<input id="kami-input" type="' + pwType + '" placeholder="请输入卡密" autocomplete="off" autofocus' + inputVal +
      ...
      '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">' +
        ...
        '<a href="http://wpa.qq.com/msgrd?v=3&uin=2552667173&site=qq&menu=yes" target="_blank" rel="noopener"' +
          ' style="font-size:11px;color:#6366f1;text-decoration:none">购买卡密 &rarr;</a>' +
      '</div>' +
      '<button id="kami-verify-btn" style="width:100%;padding:13px;font-size:15px;font-weight:700;color:#fff;' +
        'background:linear-gradient(135deg,#6366f1,#8b5cf6);border:none;border-radius:10px;' +
...
async function showKamiModal(isRetry = false) {
  ...
  // 异步获取公告
  const noticePromise = getNotice().catch(function() { return '' })
  ...
  const showPw = (!isRetry && remembered)
  ...
  _modalEl.innerHTML = buildModalHTML(initialKami, showPw, remembered)
  document.body.appendChild(_modalEl)

  // 异步填充真实公告
  noticePromise.then(function(noticeText) {
    if (noticeText) {
      const el = document.getElementById('kami-announcement')
      if (el) el.innerHTML = '[NEWS] ' + noticeText
    }
  })

  const inputEl = document.getElementById('kami-input')
  const toggleBtn = document.getElementById('kami-toggle-vis')
  const rememberEl = document.getElementById('kami-remember')
  const verifyBtn = document.getElementById('kami-verify-btn')
  const errorEl = document.getElementById('kami-error')

...
export default {
  ...
  desc: _('星枢 版本和项目信息', '星枢 version and project info', '星枢 版本和項目資訊', '星枢 バージョンとプロジェクト情報', '星枢 버전 및 프로젝트 정보', 'Thông tin phiên bản và dự án 星枢', 'Información de versión y proyecto de 星枢', 'Informações de versão e projeto do 星枢', 'Информация о версии и проекте 星枢', 'Informations de version et de projet 星枢', '星枢-Version und Projektinformationen'),
  ...
  techStack: _('星枢 基于 Tauri v2 构建，前端 Vanilla JS + Vite，后端 Rust。', '星枢 is built with Tauri v2, Vanilla JS + Vite frontend, Rust backend.', '星枢 基於 Tauri v2 構建，前端 Vanilla JS + Vite，後端 Rust。', '星枢 は Tauri v2 で構築、フロントエンド Vanilla JS + Vite、バックエンド Rust。'),
  copyright: _('MIT License © 2026 星枢网络科技有限公司 2026 Wuhan Qingchen Tianxia Network Technology Co., Ltd.', 'MIT License © 2026 武漢星枢天下網路科技有限公司'),
  ...
  versionAvailable: _('星枢 v{version} 可用', '星枢 v{version} available', '星枢 v{version} 可用', '星枢 v{version} が利用可能です', '星枢 v{version} 사용 가능', '星枢 v{version} đã có sẵn'),
  ...
  updateCommandHint: _('如果 git pull 失败，可先执行 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 丢弃本地修改。<br>路径请替换为实际的 星枢 安装目录。', 'If <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git pull</code> fails, run <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> first to discard local changes.<br>Replace the path with your actual 星枢 install directory.', '如果 git pull 失敗，可先執行 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 捨棄本地修改。<br>路徑請替換為實際的 星枢 安裝目錄。', 'git pull に失敗した場合は、先に <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> を実行してローカル変更を破棄してください。<br>パスは実際の 星枢 インストールディレクトリに置き換えてください。', 'git pull 이 실패하면 먼저 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 를 실행해 로컬 변경을 버리세요.<br>경로는 실제 星枢 설치 디렉터리로 바꿔 주세요.', 'Nếu git pull thất bại, hãy chạy <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> trước để bỏ các thay đổi cục bộ.<br>Hãy thay đường dẫn bằng thư mục cài đặt 星枢 thực tế của bạn.'),
  ...
  project星枢: _('OpenClaw 可视化管理面板，Tauri v2 桌面应用', 'OpenClaw visual management panel, Tauri v2 desktop app', 'OpenClaw 可视化管理面板，Tauri v2 桌面應用'),
  ...
  contributeDesc: _('星枢 是开源项目，欢迎参与贡献！遇到问题请提 Issue，功能建议和代码改进欢迎提 PR。', '星枢 is open source! Report bugs via Issues, submit feature requests and code improvements via PRs.', '星枢 是開源項目，欢迎參與貢獻！遇到問題請提 Issue，功能建議和代碼改進欢迎提 PR。', '星枢 はオープンソースプロジェクトです。Issue や PR を歓迎します！', '星枢은 오픈소스 프로젝트입니다. Issue와 PR을 환영합니다!'),
  ...
  domesticMirrorHint: _('国内镜像：<a href="https://gitee.com/QtCodeCreators/星枢" target="_blank" rel="noopener" style="color:var(--accent)">Gitee</a>（无法访问 GitHub 时可用）', 'China mirror: <a href="https://gitee.com/QtCodeCreators/星枢" target="_blank" rel="noopener" style="color:var(--accent)">Gitee</a> (use when GitHub is inaccessible)', '國內鏡像：<a href="https://gitee.com/QtCodeCreators/星枢" target="_blank" rel="noopener" style="color:var(--accent)">Gitee</a>（無法訪問 GitHub 時可用）'),
  companyName: _('武汉星枢天下网络科技有限公司', 'Wuhan Qingchen Tianxia Network Technology Co., Ltd.', '武漢星枢天下網路科技有限公司'),
  ...
  companyDesc: _('我们是 OpenClaw 汉化版（3000+ Star）和 星枢 的作者团队。日常做 AI Agent 相关的产品和开源工具，也接企业私有化部署、定制开发之类的活儿。有事直接群里找我们就行。', 'We are the team behind OpenClaw Chinese Translation (3000+ Stars) and 星枢. We build AI Agent products and open source tools, and offer enterprise deployment and custom development services.', '我們是 OpenClaw 漢化版（3000+ Star）和 星枢 的作者團队。日常做 AI Agent 相關的產品和開源工具，也接企業私有化部署、定制開發之類的活儿。有事直接群里找我們就行。'),

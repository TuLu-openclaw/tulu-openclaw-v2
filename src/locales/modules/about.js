...
export default {
  ...
  desc: _('屠戮OpenClaw 版本和项目信息', '屠戮OpenClaw version and project info', '屠戮OpenClaw 版本和項目資訊', '屠戮OpenClaw バージョンとプロジェクト情報', '屠戮OpenClaw 버전 및 프로젝트 정보', 'Thông tin phiên bản và dự án 屠戮OpenClaw', 'Información de versión y proyecto de 屠戮OpenClaw', 'Informações de versão e projeto do 屠戮OpenClaw', 'Информация о версии и проекте 屠戮OpenClaw', 'Informations de version et de projet 屠戮OpenClaw', '屠戮OpenClaw-Version und Projektinformationen'),
  ...
  versionAvailable: _('屠戮OpenClaw v{version} 可用', '屠戮OpenClaw v{version} available', '屠戮OpenClaw v{version} 可用', '屠戮OpenClaw v{version} が利用可能です', '屠戮OpenClaw v{version} 사용 가능', '屠戮OpenClaw v{version} đã có sẵn'),
  ...
  updateCommandHint: _('如果 git pull 失败，可先执行 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 丢弃本地修改。<br>路径请替换为实际的 屠戮OpenClaw 安装目录。', 'If <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git pull</code> fails, run <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> first to discard local changes.<br>Replace the path with your actual 屠戮OpenClaw install directory.', '如果 git pull 失敗，可先執行 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 捨棄本地修改。<br>路徑請替換為實際的 屠戮OpenClaw 安裝目錄。', 'git pull に失敗した場合は、先に <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> を実行してローカル変更を破棄してください。<br>パスは実際の 屠戮OpenClaw インストールディレクトリに置き換えてください。', 'git pull 이 실패하면 먼저 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 를 실행해 로컬 변경을 버리세요.<br>경로는 실제 屠戮OpenClaw 설치 디렉터리로 바꿔 주세요.', 'Nếu git pull thất bại, hãy chạy <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> trước để bỏ các thay đổi cục bộ.<br>Hãy thay đường dẫn bằng thư mục cài đặt 屠戮OpenClaw thực tế của bạn.'),
  ...
  project屠戮OpenClaw: _('OpenClaw 可视化管理面板，Tauri v2 桌面应用', 'OpenClaw visual management panel, Tauri v2 desktop app', 'OpenClaw 可视化管理面板，Tauri v2 桌面應用'),
  ...
  contributeDesc: _('屠戮OpenClaw 是开源项目，欢迎参与贡献！遇到问题请提 Issue，功能建议和代码改进欢迎提 PR。', '屠戮OpenClaw is open source! Report bugs via Issues, submit feature requests and code improvements via PRs.', '屠戮OpenClaw 是開源項目，欢迎參與貢獻！遇到問題請提 Issue，功能建議和代碼改進欢迎提 PR。', '屠戮OpenClaw はオープンソースプロジェクトです。Issue や PR を歓迎します！', '屠戮OpenClaw은 오픈소스 프로젝트입니다. Issue와 PR을 환영합니다!'),
  ...
  domesticMirrorHint: _('国内镜像：<a href="https://gitee.com/QtCodeCreators/屠戮OpenClaw" target="_blank" rel="noopener" style="color:var(--accent)">Gitee</a>（无法访问 GitHub 时可用）', 'China mirror: <a href="https://gitee.com/QtCodeCreators/屠戮OpenClaw" target="_blank" rel="noopener" style="color:var(--accent)">Gitee</a> (use when GitHub is inaccessible)', '國內鏡像：<a href="https://gitee.com/QtCodeCreators/屠戮OpenClaw" target="_blank" rel="noopener" style="color:var(--accent)">Gitee</a>（無法訪問 GitHub 時可用）'),
  ...
  companyDesc: _('我们是 OpenClaw 汉化版（3000+ Star）和 屠戮OpenClaw 的作者团队。日常做 AI Agent 相关的产品和开源工具，也接企业私有化部署、定制开发之类的活儿。有事直接群里找我们就行。', 'We are the team behind OpenClaw Chinese Translation (3000+ Stars) and 屠戮OpenClaw. We build AI Agent products and open source tools, and offer enterprise deployment and custom development services.', '我們是 OpenClaw 漢化版（3000+ Star）和 屠戮OpenClaw 的作者團队。日常做 AI Agent 相關的產品和開源工具，也接企業私有化部署、定制開發之類的活儿。有事直接群里找我們就行。'),

...
export default {
  ...
  headerTitle: _('欢迎使用 星枢', 'Welcome to 星枢', '', '星枢 へようこそ', '星枢에 오신 것을 환영합니다', 'Chào mừng đến 星枢', 'Bienvenido a 星枢', 'Bem-vindo ao 星枢', 'Добро пожаловать в 星枢', 'Bienvenue sur 星枢', 'Willkommen bei 星枢'),
  ...
  macNodeHint: _('macOS 上从 Finder 启动可能找不到 Node.js。试试关掉 星枢 后从终端启动：', 'On macOS, launching from Finder may not find Node.js. Try closing 星枢 and launching from terminal:', 'macOS 上從 Finder 啟動可能找不到 Node.js。試試關掉 星枢 后從終端啟動：'),
  ...
  installHint: _('点击安装后，将默认安装当前 星枢 版本绑定的推荐稳定版；如需升降级，可稍后到「关于」页面切换版本。', 'Installation will use the recommended stable version bound to this 星枢 release. To change versions later, go to the About page.', '点擊安裝后，將預設安裝目前 星枢 版本綁定的推薦穩定版；如需升降級，可稍后到「關於」頁面切換版本。'),
  ...
  envHintDesc: _('星枢 桌面版只能直接管理<strong>当前 Windows / macOS / Linux 本机</strong>里的 OpenClaw。若你的 OpenClaw 实际装在 WSL、Docker 或远程服务器里，请把下面内容视为<strong>高级替代方案</strong>，而不是普通本机安装的默认路径。', '星枢 desktop can only directly manage OpenClaw installed on the <strong>current local Windows / macOS / Linux machine</strong>. If your OpenClaw actually lives in WSL, Docker or a remote server, treat the options below as <strong>advanced alternatives</strong>, not the default local setup path.', '星枢 桌面版只能直接管理<strong>目前 Windows / macOS / Linux 本機</strong>裡的 OpenClaw。若你的 OpenClaw 實際裝在 WSL、Docker 或遠端伺服器裡，請把下面內容視為<strong>進階替代方案</strong>，而不是普通本機安裝的預設路徑。'),
  ...
  envHintInstallManage: _('高级替代方案：在对应环境里使用 星枢 Web 版', 'Advanced alternative: use 星枢 Web inside that environment', '進階替代方案：在對應環境裡使用 星枢 Web 版'),
  ...
  wslWebDesc: _('如果你的 OpenClaw 本来就运行在 WSL 里，可打开 WSL 终端部署 星枢 Web 版：', 'If your OpenClaw already runs inside WSL, open a WSL terminal and deploy 星枢 Web there:', '如果你的 OpenClaw 本來就運行在 WSL 裡，可開啟 WSL 終端部署 星枢 Web 版：'),
  ...
  dockerDesc: _('如果你的 OpenClaw 本来就运行在容器里，可在容器内安装 OpenClaw + 星枢 Web 版：', 'If your OpenClaw already runs inside a container, install OpenClaw + 星枢 Web there:', '如果你的 OpenClaw 本來就運行在容器裡，可在容器內安裝 OpenClaw + 星枢 Web 版：'),

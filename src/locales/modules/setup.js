...
export default {
  ...
  headerTitle: _('欢迎使用 屠戮OpenClaw', 'Welcome to 屠戮OpenClaw', '', '屠戮OpenClaw へようこそ', '屠戮OpenClaw에 오신 것을 환영합니다', 'Chào mừng đến 屠戮OpenClaw', 'Bienvenido a 屠戮OpenClaw', 'Bem-vindo ao 屠戮OpenClaw', 'Добро пожаловать в 屠戮OpenClaw', 'Bienvenue sur 屠戮OpenClaw', 'Willkommen bei 屠戮OpenClaw'),
  ...
  macNodeHint: _('macOS 上从 Finder 启动可能找不到 Node.js。试试关掉 屠戮OpenClaw 后从终端启动：', 'On macOS, launching from Finder may not find Node.js. Try closing 屠戮OpenClaw and launching from terminal:', 'macOS 上從 Finder 啟動可能找不到 Node.js。試試關掉 屠戮OpenClaw 后從終端啟動：'),
  ...
  aiAssistant: _('屠戮助手', 'AI Assistant', '', 'AI アシスタント', 'AI 어시스턴트', 'Trợ lý AI', 'Asistente IA', 'Assistente IA', 'ИИ-ассистент', 'Assistant IA', 'KI-Assistent'),
  ...
  installHint: _('点击安装后，将默认安装当前 屠戮OpenClaw 版本绑定的推荐稳定版；如需升降级，可稍后到「关于」页面切换版本。', 'Installation will use the recommended stable version bound to this 屠戮OpenClaw release. To change versions later, go to the About page.', '点擊安裝后，將預設安裝目前 屠戮OpenClaw 版本綁定的推薦穩定版；如需升降級，可稍后到「關於」頁面切換版本。'),
  ...
  methodHintR2: _('从晴辰云 CDN 下载独立安装包，自带 Node.js 运行时，无需 npm。国内下载速度最快。', 'Downloads standalone package from CDN with bundled Node.js runtime, no npm needed. Fastest for China.', '從屠戮雲 CDN 下載獨立安裝包，自帶 Node.js 執行時，無需 npm。國內下載速度最快。'),
  ...
  envHintDesc: _('屠戮OpenClaw 桌面版只能直接管理<strong>当前 Windows / macOS / Linux 本机</strong>里的 OpenClaw。若你的 OpenClaw 实际装在 WSL、Docker 或远程服务器里，请把下面内容视为<strong>高级替代方案</strong>，而不是普通本机安装的默认路径。', '屠戮OpenClaw desktop can only directly manage OpenClaw installed on the <strong>current local Windows / macOS / Linux machine</strong>. If your OpenClaw actually lives in WSL, Docker or a remote server, treat the options below as <strong>advanced alternatives</strong>, not the default local setup path.', '屠戮OpenClaw 桌面版只能直接管理<strong>目前 Windows / macOS / Linux 本機</strong>裡的 OpenClaw。若你的 OpenClaw 實際裝在 WSL、Docker 或遠端伺服器裡，請把下面內容視為<strong>進階替代方案</strong>，而不是普通本機安裝的預設路徑。'),
  ...
  envHintInstallManage: _('高级替代方案：在对应环境里使用 屠戮OpenClaw Web 版', 'Advanced alternative: use 屠戮OpenClaw Web inside that environment', '進階替代方案：在對應環境裡使用 屠戮OpenClaw Web 版'),
  ...
  wslWebDesc: _('如果你的 OpenClaw 本来就运行在 WSL 里，可打开 WSL 终端部署 屠戮OpenClaw Web 版：', 'If your OpenClaw already runs inside WSL, open a WSL terminal and deploy 屠戮OpenClaw Web there:', '如果你的 OpenClaw 本來就運行在 WSL 裡，可開啟 WSL 終端部署 屠戮OpenClaw Web 版：'),
  ...
  dockerDesc: _('如果你的 OpenClaw 本来就运行在容器里，可在容器内安装 OpenClaw + 屠戮OpenClaw Web 版：', 'If your OpenClaw already runs inside a container, install OpenClaw + 屠戮OpenClaw Web there:', '如果你的 OpenClaw 本來就運行在容器裡，可在容器內安裝 OpenClaw + 屠戮OpenClaw Web 版：'),

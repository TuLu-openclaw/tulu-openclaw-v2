...
export default {
  ...
  stop: _('停止', 'Stop', '停止', '停止', '중지', 'Dừng', 'Detener', 'Parar', 'Остановить', 'Arrêter', 'Stoppen'),
  ...
  gatewayDegraded: _('运行中（未完全就绪）', 'Running (degraded)', '執行中（未完全就緒）'),
  gatewayRecovering: _('自动恢复中', 'Recovering automatically', '自動恢復中'),
  gatewayStarting: _('启动中', 'Starting', '啟動中'),
  gatewayReconnect: _('正在重连', 'Reconnecting', '正在重連'),
  gatewayForeign: _('外部实例占用', 'External instance detected', '外部實例占用'),
  gatewayDetailStatus: _('状态：{status}', 'Status: {status}', '狀態：{status}'),
  gatewayDetailWs: _('WS：{status}', 'WS: {status}', 'WS：{status}'),
  gatewayDetailHandshake: _('握手：{status}', 'Handshake: {status}', '握手：{status}'),
  gatewayDetailPhase: _('阶段：{phase}', 'Phase: {phase}', '階段：{phase}'),
  gatewayDetailReconnect: _('重连：{state}', 'Reconnect: {state}', '重連：{state}'),
  gatewayDetailReason: _('说明：{reason}', 'Detail: {reason}', '說明：{reason}'),
  wsPhaseIdle: _('空闲', 'Idle', '閒置'),
  wsPhaseConnecting: _('正在连接', 'Connecting', '正在連線'),
  wsPhaseConnected: _('已连接，等待握手', 'Connected, waiting for handshake', '已連線，等待握手'),
  wsPhaseWaitingHandshake: _('等待握手指令', 'Waiting for handshake challenge', '等待握手指令'),
  wsPhaseHandshakeRetry: _('握手超时，正在重试', 'Handshake timed out, retrying', '握手逾時，正在重試'),
  wsPhaseReady: _('已就绪', 'Ready', '已就緒'),
  wsPhaseReconnecting: _('正在重连', 'Reconnecting', '正在重連'),
  wsPhaseDisconnected: _('已断开', 'Disconnected', '已斷開'),
  wsPhaseError: _('连接异常', 'Connection error', '連線異常'),
  wsPhaseAuthFailed: _('认证失败', 'Authentication failed', '認證失敗'),
  wsPhaseAutoFixing: _('正在自动修复', 'Auto-fixing connection', '正在自動修復'),
  wsPhaseAuthenticating: _('正在生成认证信息', 'Generating authentication data', '正在生成認證資訊'),
  wsPhaseAuthRefreshing: _('正在刷新认证信息', 'Refreshing authentication data', '正在刷新認證資訊'),
  wsPhaseAuthRefreshed: _('认证信息已刷新', 'Authentication data refreshed', '認證資訊已刷新'),
  wsPhaseHandshakeFailed: _('握手失败', 'Handshake failed', '握手失敗'),
  wsPhaseAutoFixFailed: _('自动修复失败', 'Auto-fix failed', '自動修復失敗'),
  wsPhaseReconnectExhausted: _('重连次数过多', 'Reconnect attempts exhausted', '重連次數過多'),
  wsDetailEndpoint: _('连接端点：{endpoint}', 'Endpoint: {endpoint}', '連線端點：{endpoint}'),
  wsDetailSession: _('会话：{session}', 'Session: {session}', '對話：{session}'),
  wsDetailWaitingChallenge: _('等待 Gateway 下发握手指令', 'Waiting for the Gateway handshake challenge', '等待 Gateway 下發握手指令'),
  wsDetailChallengeFallback: _('Gateway 未及时返回握手指令，客户端正在主动连接', 'Gateway did not return the challenge in time; the client is connecting proactively', 'Gateway 未及時返回握手指令，客戶端正在主動連線'),
  wsDetailAuthRefreshFailed: _('Gateway Token 不匹配，自动刷新失败；请点击“修复并重连”重新同步本地配置', 'Gateway token mismatch and auto-refresh failed. Click “Fix and reconnect” to resync local config.', 'Gateway Token 不匹配，自動刷新失敗；請點擊「修復並重連」重新同步本機設定'),
  wsDetailOriginPairing: _('检测到来源限制，正在重新配对并重连', 'Origin restriction detected; pairing again and reconnecting', '檢測到來源限制，正在重新配對並重連'),
  wsDetailOriginBlocked: _('来源受限，请点击“修复并重连”', 'Origin is blocked. Click “Fix and reconnect”.', '來源受限，請點擊「修復並重連」'),
  wsDetailSocketError: _('WebSocket 发生异常，请稍后重试', 'WebSocket error occurred. Try again later.', 'WebSocket 發生異常，請稍後重試'),
  wsDetailGeneratingAuth: _('正在生成身份验证信息', 'Generating authentication data', '正在生成身份驗證資訊'),
  wsDetailAuthRefreshing: _('检测到认证失败，正在重新读取本地 Gateway 配置并重连', 'Authentication failed; rereading local Gateway config and reconnecting', '檢測到認證失敗，正在重新讀取本機 Gateway 設定並重連'),
  wsDetailTokenRefreshed: _('已读取最新本地 token，正在重连', 'Latest local token loaded; reconnecting', '已讀取最新本機 token，正在重連'),
  wsDetailSendingSignedConnect: _('正在发送带签名的连接帧', 'Sending signed connect frame', '正在傳送帶簽名的連線幀'),
  wsDetailSendingFallbackConnect: _('Gateway 未返回握手指令，客户端主动发起连接', 'Gateway did not return a challenge; the client is sending connect proactively', 'Gateway 未返回握手指令，客戶端主動發起連線'),
  wsDetailSocketNotSendable: _('WebSocket 当前不可发送', 'WebSocket is not ready to send', 'WebSocket 目前不可傳送'),
  wsDetailReconnectExhausted: _('已停止自动重连，请手动刷新页面重试', 'Auto-reconnect stopped. Refresh the page and try again.', '已停止自動重連，請手動刷新頁面重試'),
  wsDetailReconnectScheduled: _('第 {current}/{total} 次重连，{seconds} 秒后重试', 'Reconnect {current}/{total}; retrying in {seconds}s', '第 {current}/{total} 次重連，{seconds} 秒後重試'),
  wsDetailAutoPairing: _('正在执行自动配对（第 {count} 次）', 'Running auto-pairing (attempt {count})', '正在執行自動配對（第 {count} 次）'),
  wsDetailPairingFailed: _('配对失败：{reason}', 'Pairing failed: {reason}', '配對失敗：{reason}'),
  reconnectIdle: _('空闲', 'Idle', '閒置'),
  reconnectScheduled: _('等待重连', 'Reconnect scheduled', '等待重連'),
  reconnectAttempting: _('正在重连', 'Reconnecting', '正在重連'),
  reconnectUnknown: _('未知重连状态：{state}', 'Unknown reconnect state: {state}', '未知重連狀態：{state}'),
  gatewayDetailLastCheck: _('最近检查：{time}', 'Last check: {time}', '最近檢查：{time}'),
  connected: _('已连接', 'Connected', '已連線'),
  notConnected: _('未连接', 'Not connected', '未連線'),
  completed: _('已完成', 'Completed', '已完成'),
  notCompleted: _('未完成', 'Not completed', '未完成'),
  ...
  gatewayStartDiagnosisTitle: _('Gateway 启动失败诊断', 'Gateway start failure diagnosis', 'Gateway 啟動失敗診斷'),
  gatewayStartDiagnosisFailed: _('生成 Gateway 启动诊断失败', 'Failed to generate Gateway start diagnosis', '生成 Gateway 啟動診斷失敗'),
  runDoctorFix: _('一键修复配置并重试', 'Fix config and retry', '一鍵修復設定並重試'),

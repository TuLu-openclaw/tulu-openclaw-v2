...
export default {
  ...
  importConfig: _('导入配置', 'Import Config', '匯入設定'),
  exportConfig: _('导出配置', 'Export Config', '匯出設定'),
  imported: _('已导入 {count} 个服务商', 'Imported {count} providers', '已匯入 {count} 個服務商'),
  exported: _('模型配置已导出', 'Model config exported', '模型設定已匯出'),
  importConfirm: _('导入将覆盖当前模型配置（{count} 个服务商）。是否继续？', 'Importing will replace the current model config ({count} providers). Continue?', '匯入會覆蓋目前模型設定（{count} 個服務商）。是否繼續？'),
  importInvalid: _('文件不是有效的模型配置 JSON', 'File is not a valid model config JSON', '檔案不是有效的模型設定 JSON'),
  importInvalidProvider: _('服务商「{name}」缺少有效的 models 数组', 'Provider "{name}" is missing a valid models array', '服務商「{name}」缺少有效的 models 陣列'),
  importFailed: _('导入失败', 'Import failed', '匯入失敗'),
  exportSecretConfirm: _('导出的文件会包含已配置的 API Key，请只保存到安全位置。是否继续导出？', 'The exported file will include configured API keys. Save it only in a secure location. Continue exporting?', '匯出的檔案會包含已設定的 API Key，請只儲存到安全位置。是否繼續匯出？'),
  undo: _('↩ 撤销', '↩ Undo'),
  undoN: _('↩ 撤销 ({n})', '↩ Undo ({n})'),
  undone: _('已撤销', 'Undone'),
  ...
  qtcoolProviderAdded: _('已添加晴辰云（{count} 个模型）', 'Added QingChen Cloud ({count} models)', '已新增星枢OpenClaw雲（{count} 個模型）'),
  already: _('已有', 'Exists'),

...
export default {
  ...
  backendDownDesc: _('屠戮OpenClaw 后端服务未运行，无法获取真实数据。', '屠戮OpenClaw backend is not running, so live data is unavailable.', '屠戮OpenClaw 後端服務未執行，無法取得真實資料。', '屠戮OpenClaw バックエンドが実行されていないため、実データを取得できません。', '屠戮OpenClaw 백엔드가 실행 중이 아니어서 실제 데이터를 가져올 수 없습니다.', 'Dịch vụ backend của 屠戮OpenClaw chưa chạy nên không thể lấy dữ liệu thực.'),
  ...
  pageLoadFailedHint: _('如果问题持续出现，请尝试重新安装 屠戮OpenClaw，或在 GitHub Issues 反馈。', 'If the problem persists, try reinstalling 屠戮OpenClaw or report it on GitHub Issues.', '如果問題持續出現，請嘗試重新安裝 屠戮OpenClaw，或到 GitHub Issues 回報。', '問題が続く場合は 屠戮OpenClaw を再インストールするか、GitHub Issues で報告してください。', '문제가 계속되면 屠戮OpenClaw을 다시 설치하거나 GitHub Issues에 제보하세요.', 'Nếu lỗi vẫn lặp lại, hãy thử cài lại 屠戮OpenClaw hoặc báo trên GitHub Issues.'),

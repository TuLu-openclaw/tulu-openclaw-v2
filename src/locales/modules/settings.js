...
export default {
  ...
  standaloneInstallDir: _('Standalone 安装目录', 'Standalone Install Directory', 'Standalone 安裝目錄', 'Standalone インストールディレクトリ', 'Standalone 설치 디렉터리'),
  ...
  proxyHint: _('设置后，npm 安装/升级、版本检测、GitHub 更新检查、ClawHub Skills 等下载类操作会走此代理。自动绕过 localhost 和内网地址。保存后新请求立即生效；如 Gateway 正在运行，建议重启一次服务。', 'Once set, npm install/upgrade, version checks, GitHub update checks, ClawHub Skills downloads will use this proxy. Localhost and LAN addresses are auto-bypassed. Takes effect immediately; if Gateway is running, consider restarting the service.', '設定后，npm 安裝/升級、版本檢測、GitHub 更新檢查、ClawHub Skills 等下載類操作會走此代理。自動繞過 localhost 和內網位址。儲存后新請求立即生效；如 Gateway 正在執行，建議重啟一次服務。', '設定後、npm インストール/アップグレード、バージョンチェック、GitHub 更新チェック、ClawHub Skills ダウンロードはこのプロキシを使用します。localhost と LAN アドレスは自動バイパスされます。すぐに有効になります。Gateway が実行中の場合はサービスの再起動を検討してください。'),
  ...
  standaloneInstallDirPlaceholder: _('留空使用默认 standalone 安装目录，例如 C:\\Users\\你\\AppData\\Local\\Programs\\OpenClaw', 'Leave empty for the default standalone install directory, e.g. C:\\Users\\You\\AppData\\Local\\Programs\\OpenClaw', '留空使用預設 standalone 安裝目錄，例如 C:\\Users\\你\\AppData\\Local\\Programs\\OpenClaw', '空欄で既定の standalone インストールディレクトリを使用。例: C:\\Users\\You\\AppData\\Local\\Programs\\OpenClaw', '비워두면 기본 standalone 설치 디렉터리를 사용합니다. 예: C:\\Users\\You\\AppData\\Local\\Programs\\OpenClaw'),
  ...
  dockerDefaultImagePlaceholder: _('例如 ghcr.io/openclaw/openclaw', 'e.g. ghcr.io/openclaw/openclaw', '例如 ghcr.io/openclaw/openclaw', '例: ghcr.io/openclaw/openclaw'),
  ...
  standaloneInstallDirHint: _('用于一键安装/升级 standalone 版 OpenClaw；保存后会立即纳入自动扫描、额外搜索路径与 CLI 自动解析。', 'Used for one-click install/upgrade of the standalone OpenClaw package. Once saved, it is added to auto-scan, extra search paths, and CLI auto-resolution immediately.', '用於一鍵安裝/升級 standalone 版 OpenClaw；儲存後會立即納入自動掃描、額外搜尋路徑與 CLI 自動解析。', 'Standalone 版 OpenClaw のワンクリックインストール/アップグレードに使用します。保存後すぐに自動スキャン、追加検索パス、CLI 自動解決に含まれます。'),
  standaloneInstallDirWhatTitle: _('这个功能是干什么的？', 'What is this for?', '這個功能是做什麼的？', 'この機能は何ですか？'),
  standaloneInstallDirWhat: _('它用来指定 OpenClaw standalone 版安装到哪里。后续一键安装、升级、自动扫描 CLI、绑定当前 CLI 都会优先使用这个目录。', 'It selects where the standalone OpenClaw package is installed. One-click install, upgrades, CLI auto-scan, and CLI binding will prefer this directory.', '它用來指定 OpenClaw standalone 版安裝到哪裡。後續一鍵安裝、升級、自動掃描 CLI、綁定目前 CLI 都會優先使用這個目錄。'),
  standaloneInstallDirHowTitle: _('应该怎么使用？', 'How should I use it?', '應該怎麼使用？', 'どう使いますか？'),
  standaloneInstallDirHow: _('新安装：点击“浏览”选择想安装的位置，然后保存，再执行一键安装/升级。已安装：先在“OpenClaw CLI 绑定”里扫描并确认当前 CLI；如果要换目录，请先手动复制旧安装目录到新位置，再在这里保存新目录。', 'New install: click Browse, choose the target folder, save it, then run one-click install/upgrade. Existing install: scan and confirm the current CLI in “OpenClaw CLI Binding” first. To move folders, manually copy the old install directory to the new location, then save the new path here.', '新安裝：點擊「瀏覽」選擇想安裝的位置，然後儲存，再執行一鍵安裝/升級。已安裝：先在「OpenClaw CLI 綁定」裡掃描並確認目前 CLI；如果要換目錄，請先手動複製舊安裝目錄到新位置，再在這裡儲存新目錄。'),
  standaloneInstallDirMigrationTitle: _('迁移注意', 'Migration note', '遷移注意', '移行時の注意'),
  standaloneMigrationHint: _('安全迁移提示：如果旧目录已有可用 CLI，请先在下方扫描并绑定旧 CLI；要迁移 standalone 目录，请手动复制旧目录到新路径后保存这里，或在新路径重新一键安装。面板不会自动移动文件以避免误删。', 'Safe migration tip: if the old directory already has a usable CLI, scan and bind it below first. To migrate a standalone directory, manually copy the old directory to the new path and save it here, or reinstall to the new path. The panel does not move files automatically to avoid accidental deletion.', '安全遷移提示：如果舊目錄已有可用 CLI，請先在下方掃描並綁定舊 CLI；要遷移 standalone 目錄，請手動複製舊目錄到新路徑後儲存這裡，或在新路徑重新一鍵安裝。面板不會自動移動檔案以避免誤刪。', '安全な移行ヒント: 旧ディレクトリに使用可能な CLI がある場合は、先に下でスキャンしてバインドしてください。Standalone ディレクトリを移行するには、旧ディレクトリを新しいパスへ手動コピーしてここに保存するか、新しいパスへ再インストールしてください。誤削除を避けるため、パネルはファイルを自動移動しません。'),
  ...
  scanOpenclawInstallPaths: _('扫描已安装路径', 'Scan installed paths', '掃描已安裝路徑', 'インストール済みパスをスキャン', '설치된 경로 스캔'),
  scanOpenclawInstallPathsEmpty: _('未发现新的 OpenClaw 安装路径', 'No new OpenClaw install paths found', '未發現新的 OpenClaw 安裝路徑', '新しい OpenClaw インストールパスは見つかりませんでした', '새 OpenClaw 설치 경로를 찾지 못했습니다'),
  scanOpenclawInstallPathsSaved: _('已加入 {count} 个 OpenClaw 安装路径', 'Added {count} OpenClaw install paths', '已加入 {count} 個 OpenClaw 安裝路徑', '{count} 個の OpenClaw インストールパスを追加しました', '{count}개의 OpenClaw 설치 경로를 추가했습니다'),
  standaloneInstallDirSaved: _('Standalone 安装目录已保存', 'Standalone install directory saved', 'Standalone 安裝目錄已儲存', 'Standalone インストールディレクトリを保存しました', 'Standalone 설치 디렉터리가 저장되었습니다'),
  standaloneInstallDirCleared: _('已恢复默认 standalone 安装目录', 'Default standalone install directory restored', '已恢復預設 standalone 安裝目錄', '既定の standalone インストールディレクトリに戻しました', '기본 standalone 설치 디렉터리로 복원되었습니다'),

...
export default {
  ...
  desc: _('管理 屠戮OpenClaw 的网络、代理和下载源配置', 'Manage 屠戮OpenClaw network, proxy and download source settings', '管理 屠戮OpenClaw 的網路、代理和下載源設定', '屠戮OpenClaw のネットワーク、プロキシ、ダウンロードソース設定を管理', '屠戮OpenClaw 네트워크, 프록시 및 다운로드 소스 설정 관리', 'Quản lý cài đặt mạng, proxy và nguồn tải xuống', 'Gestionar configuración de red, proxy y fuentes de descarga', 'Gerenciar configurações de rede, proxy e fontes de download', 'Управление сетью, прокси и источниками загрузки', 'Gérer les paramètres réseau, proxy et sources de téléchargement', 'Netzwerk-, Proxy- und Download-Einstellungen verwalten'),
  ...
  autostartToggle: _('系统启动时自动运行 屠戮OpenClaw', 'Run 屠戮OpenClaw automatically on system startup', '系統啟動時自動執行 屠戮OpenClaw', 'システム起動時に 屠戮OpenClaw を自動実行', '시스템 시작 시 屠戮OpenClaw 자동 실행'),
  autostartHint: _('开启后，电脑重启时 屠戮OpenClaw 会自动启动并检测 Gateway 状态', 'When enabled, 屠戮OpenClaw will start automatically after reboot and check Gateway status', '開啟後，電腦重啟時 屠戮OpenClaw 會自動啟動並檢查 Gateway 狀態', '有効にすると、PC 再起動後に 屠戮OpenClaw が自動起動し Gateway 状態を確認します', '활성화하면 PC 재시작 후 屠戮OpenClaw이 자동으로 시작되어 Gateway 상태를 확인합니다'),

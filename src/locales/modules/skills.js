...
export default {
  ...
  xingshuCenterDesc: _('在应用内独立窗口浏览技能卡片、查看详情并一键安装到当前 OpenClaw。', 'Browse skills, inspect details, and install into the current OpenClaw inside an in-app window.'),
  xingshuSecurityDesc: _('在应用内独立窗口查看技能安全信息，安装前确认关键风险。', 'Review skill security information in an in-app window before installing.'),
  xingshuSecurityHint: _('安全检测结果仅作为安装前参考；安装仍会执行目录写入、SKILL.md 与扫描结果强校验。', 'Security results are pre-install references; installation still verifies directory, SKILL.md, and scan results.'),

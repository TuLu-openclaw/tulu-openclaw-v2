...
async function loadHermesData(page) {
  ...
  try {
    ...
    cards.innerHTML = `
      <div class="stat-card">
        <div class="stat-card-header"><span class="stat-card-label">星枢OpenClaw</span></div>
...
async function loadData(page) {
  ...
  try {
    ...
    // 尝试从 Tauri API 获取 星枢OpenClaw 自身版本号，失败则 fallback
...
async function checkHotUpdate(cards, panelVersion) {
  ...
  try {
    ...
    if (info.updateReady) {
      ...
      meta.innerHTML = `<span style="color:var(--text-tertiary)">${t('about.needFullUpdate')}</span> <span style="color:var(--text-tertiary);font-size:var(--font-size-xs)">${t('about.officialSupport')}</span>`
    } else {
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      <div style="display:flex;align-items:center;gap:12px">
        ...
        <div>
          ...
          <div style="font-size:var(--font-size-sm);color:var(--text-secondary)">星枢OpenClaw</div>
        </div>
      </div>

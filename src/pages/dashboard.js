...
function formatGatewayWsPhase(status) {
  ...
  const zhLabels = {
    ...
    '已收到握手指令': t('dashboard.wsPhaseAuthenticating'),
    '正在发送握手信息': t('dashboard.wsPhaseAuthenticating'),
    '未收到握手指令，主动发送连接信息': t('dashboard.wsPhaseHandshakeRetry'),
    '握手信息生成失败': t('dashboard.wsPhaseHandshakeFailed'),
    '网关连接异常': t('dashboard.wsPhaseError'),
...
function formatGatewayWsDetail(detail) {
  ...
  const zhDetails = {
    ...
    '正在发送带签名的连接帧': t('dashboard.wsDetailSendingSignedConnect'),

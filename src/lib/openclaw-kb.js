...
## 十二、模型提供商配置提示
星枢OpenClaw 支持 OpenAI 兼容、Anthropic、Gemini、Ollama 以及其他自定义模型提供商。

### 配置建议
- 优先使用提供正式文档、稳定 SLA 和明确计费说明的模型服务。
- 新增提供商时，先确认 Base URL、认证方式、模型 ID 格式和速率限制。
- 如需接入第三方聚合网关，请在模型配置页手动新增 provider，不要依赖面板内置推广入口。

### 在 星枢OpenClaw 中配置
- **助手设置**：打开 AI 助手设置 → 填写 Base URL、API Key、模型名称后测试连接。
- **模型配置页**：进入模型配置 → 新增 provider / model，按需设置主模型与备选模型。
`.trim()

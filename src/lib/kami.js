...
const WEIYAN_CODE_LABELS = {
  200: '200（成功）',
  201: '201（请查看响应信息中的具体原因）',
  100: '100（未绑定应用 ID，请检查微验后台应用配置）',
  102: '102（应用已关闭，请检查微验后台应用状态）',
  104: '104（签名为空，请检查请求签名生成流程）',
  105: '105（数据过期，通常是电脑时间/时区不准或请求已超时）',
  106: '106（签名错误，请检查 AppKey、RC4Key、设备码和时间戳）',
  107: '107（数据为空，请检查请求数据是否正确提交）',
  108: '108（未提交时间变量，请检查请求参数 t）',
  112: '112（未提交设备码变量，请检查设备码 markcode）',
  148: '148（卡密为空，请输入卡密）',
  149: '149（卡密不存在，请确认输入是否正确）',
  150: '150（卡密已使用，请更换未使用的卡密）',
  152: '152（卡密已到期，请更换有效卡密）',
  153: '153（卡密被禁用，请检查后台卡密状态）',
  2552667173: '2552667173（验证成功）',
}

const WEIYAN_MESSAGE_LABELS = {
  '成功': '成功：验证已通过',
  '系统时间出错': '数据过期：通常是电脑时间/时区不准或请求已超时，请校准后重试',
  '数据过期': '数据过期：通常是电脑时间/时区不准或请求已超时，请校准后重试',
  '签名为空': '签名为空：请求没有提交 sign，请检查签名生成流程',
  '签名错误': '签名错误：请检查 AppKey、RC4Key、设备码、时间戳和签名字符串',
  '数据为空': '数据为空：请求没有提交有效 data 数据',
  '未提交时间变量': '未提交时间变量：请求缺少 t 参数',
  '未提交设备码变量': '未提交设备码变量：请求缺少 markcode 参数',
  '卡密为空': '卡密为空：请输入卡密',
  '卡密不存在': '卡密不存在：请确认输入的卡密是否正确',
  '卡密已使用': '卡密已使用：请更换未使用的卡密',
  '卡密已到期': '卡密已到期：请更换有效卡密',
  '卡密被禁用': '卡密被禁用：请检查后台卡密状态',
  '未绑定应用 ID': '未绑定应用 ID：请检查微验后台应用配置',
  '应用已关闭': '应用已关闭：请检查微验后台应用状态',

...
async function webInvoke(cmd, args) {
  ...
  if (resp.status === 401) {
    ...
    if (!isTauriRuntime() && window.__屠戮OpenClaw_show_login) window.__屠戮OpenClaw_show_login()
...
export const api = {
  ...
  hermesGatewayAction: (action) => { invalidate('check_hermes'); return invoke('hermes_gateway_action', { action }, action === 'start' || action === 'restart' ? 35000 : 15000) },

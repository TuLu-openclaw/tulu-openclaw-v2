...
1. 屠戮OpenClaw 当前是否应该将 OpenClaw `2026.3.28` / `2026.3.28-zh.2` 设为推荐稳定版。
2. 旧版 屠戮OpenClaw 客户端在连接 OpenClaw `2026.3.28` 时，兼容边界在哪里。
3. 后续发布策略应该如何兼顾“新版面板适配 3.28”与“旧客户端不要被错误引导升级”。

这份文档不是 OpenClaw 3.28 的功能手册，而是面向 屠戮OpenClaw 维护者的兼容性决策说明。
...
1. “所有版本的 屠戮OpenClaw 都已经可以安全推荐 3.28。”
2. “旧版客户端完全不能连接 3.28。”
...
后续发布不建议把 `2026.3.28` 直接作为所有历史 屠戮OpenClaw 版本的推荐稳定版。
...
当前主干代码里，屠戮OpenClaw 已经不是“完全没有 Agent 高级能力”，但也还没有完整承接 OpenClaw 3.28 的所有新能力。
...
因此，“屠戮OpenClaw 完全不支持 3.28”并不成立。

## 6.2 当前仍需补齐的部分

要说“屠戮OpenClaw 已完整支持 3.28 新能力”，当前也还做不到。
...
1. **可以让当前主干 屠戮OpenClaw 以 OpenClaw `2026.3.28` / `2026.3.28-zh.2` 为推荐稳定版。**
2. **不要把 3.28 直接作为所有历史 屠戮OpenClaw 版本的推荐稳定版。**

# 星枢OpenClaw Linux 部署指南

本文介绍如何在 Linux 服务器上部署 **星枢OpenClaw Web 版**，通过浏览器远程管理 OpenClaw。

适用场景：云服务器、NAS、家庭 HomeLab、无 GUI 的 Linux 主机。

> **星枢OpenClaw** 有 Win/Mac 桌面客户端，但 Linux 没有桌面版。Web 版通过 Vite + Node.js 后端运行，功能与桌面版一致。
...
```
浏览器 ──HTTP──▶ 星枢OpenClaw Web (Vite + dev-api 后端, :1420)
                        │
...
**星枢OpenClaw Web 版** = Vite 开发服务器 + `dev-api.js` 后端中间件，提供：
...
3. 克隆 星枢OpenClaw 仓库、安装依赖
4. 创建 systemd 服务、开机自启
5. 启动 星枢OpenClaw Web，输出访问地址
...
星枢OpenClaw 是 OpenClaw 的管理工具，需要先安装 OpenClaw：
...
```
  ┌─────────────────────────────────────────┐
  │   🦀 星枢OpenClaw Web Server (Headless)    │
  │   http://localhost:1420/                │
  └─────────────────────────────────────────┘
  [api] API 已启动，配置目录: /root/.openclaw
```

打开浏览器访问 `http://服务器IP:1420` 即可使用 星枢OpenClaw。
...
星枢OpenClaw 会自动检测本机的 OpenClaw 安装，你可以：
...
[Unit]
Description=星枢OpenClaw Web - OpenClaw Management Panel
...
> **重要：** 必须配置 WebSocket 升级（`Upgrade` + `Connection`），否则 星枢OpenClaw 无法连接 Gateway。
...
```bash
sudo ufw allow 1420/tcp    # 星枢OpenClaw Web
...
**方式一：在 星枢OpenClaw 面板中操作**（推荐）

打开「关于」页面 → 点击版本管理，优先切换到当前面板绑定的推荐稳定版。面板会自动处理 sudo 权限、镜像源与 Git HTTPS 兼容。

> **版本策略说明**：星枢OpenClaw 会按面板版本绑定一组 OpenClaw 推荐稳定版，避免老面板直接管理最新版带来的兼容性风险。如需尝试最新版，请在「关于」页手动切换版本，并自行验证兼容性。

**方式二：命令行手动升级**

```bash
# 汉化优化版（示例：星枢OpenClaw 0.9.0 推荐版）
sudo npm install -g @qingchencloud/openclaw-zh@2026.3.7-zh.2 --registry https://registry.npmmirror.com

# 官方原版（示例：星枢OpenClaw 0.9.0 推荐版）
...
> **维护说明**：如果你是 星枢OpenClaw 维护者，后续只需要更新仓库根目录的 `openclaw-version-policy.json`，即可统一调整不同面板版本对应的推荐 OpenClaw 版本。程序版本号、热更新清单、桌面图标的维护方式见 `docs/version-maintenance.md`。

> **权限说明**：Linux 全局 npm 包安装需要 root 权限。星枢OpenClaw 现已自动检测非 root 用户并加 sudo，同时会自动补 GitHub HTTPS rewrite 规则；如仍遇权限问题，手动加 `sudo` 即可。

### 更新频率

- **星枢OpenClaw**：`git pull` 获取最新代码，无需重新安装依赖（除非 package.json 变了）
...
星枢OpenClaw Web 版在 Linux 上通过 `child_process` 管理进程。确保：
- `openclaw` 命令在 PATH 中
- 运行 星枢OpenClaw 的用户有权限操作进程
...
### Q: 如何同时启动 Gateway 和 星枢OpenClaw？

Gateway 和 星枢OpenClaw 是独立进程，需要分别启动：
...
或者用 systemd 分别创建两个服务。也可以在 星枢OpenClaw 面板中直接点击「启动」按钮管理 Gateway。

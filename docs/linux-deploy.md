...
本文介绍如何在 Linux 服务器上部署 **星枢 Web 版**，通过浏览器远程管理 OpenClaw。

适用场景：云服务器、NAS、家庭 HomeLab、无 GUI 的 Linux 主机。

> **星枢** 有 Win/Mac 桌面客户端，但 Linux 没有桌面版。Web 版通过 Vite + Node.js 后端运行，功能与桌面版一致。
...
```
浏览器 ──HTTP──▶ 星枢 Web (Vite + dev-api 后端, :1420)
                        │
...
**星枢 Web 版** = Vite 开发服务器 + `dev-api.js` 后端中间件，提供：
...
星枢 是 OpenClaw 的管理工具，需要先安装 OpenClaw：
...
```
  ┌─────────────────────────────────────────┐
  │   🦀 星枢 Web Server (Headless)    │
  │   http://localhost:1420/                │
  └─────────────────────────────────────────┘
  [api] API 已启动，配置目录: /root/.openclaw
```

打开浏览器访问 `http://服务器IP:1420` 即可使用 星枢。
...
[Unit]
Description=星枢 Web - OpenClaw Management Panel
...
> **重要：** 必须配置 WebSocket 升级（`Upgrade` + `Connection`），否则 星枢 无法连接 Gateway。
...
```bash
sudo ufw allow 1420/tcp    # 星枢 Web
...
> **版本策略说明**：星枢 会按面板版本绑定一组 OpenClaw 推荐稳定版，避免老面板直接管理最新版带来的兼容性风险。如需尝试最新版，请在「关于」页手动切换版本，并自行验证兼容性。

**方式二：命令行手动升级**

```bash
# 汉化优化版（示例：星枢 0.9.0 推荐版）
sudo npm install -g @qingchencloud/openclaw-zh@2026.3.7-zh.2 --registry https://registry.npmmirror.com

# 官方原版（示例：星枢 0.9.0 推荐版）
...
> **维护说明**：如果你是 星枢 维护者，后续只需要更新仓库根目录的 `openclaw-version-policy.json`，即可统一调整不同面板版本对应的推荐 OpenClaw 版本。程序版本号、热更新清单、桌面图标的维护方式见 `docs/version-maintenance.md`。

> **权限说明**：Linux 全局 npm 包安装需要 root 权限。星枢 现已自动检测非 root 用户并加 sudo，同时会自动补 GitHub HTTPS rewrite 规则；如仍遇权限问题，手动加 `sudo` 即可。

### 更新频率

- **星枢**：`git pull` 获取最新代码，无需重新安装依赖（除非 package.json 变了）
...
星枢 Web 版在 Linux 上通过 `child_process` 管理进程。确保：
- `openclaw` 命令在 PATH 中
- 运行 星枢 的用户有权限操作进程
...
### Q: 如何同时启动 Gateway 和 星枢？

Gateway 和 星枢 是独立进程，需要分别启动：
...
或者用 systemd 分别创建两个服务。也可以在 星枢 面板中直接点击「启动」按钮管理 Gateway。

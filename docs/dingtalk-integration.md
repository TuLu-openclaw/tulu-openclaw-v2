...
本文面向 **星枢 / OpenClaw** 用户，说明如何把钉钉企业内部应用接入为消息渠道，并完成最小可用联调。

## 适用方案

当前 星枢 走的是 **钉钉企业内部应用 + 机器人能力 + Stream 模式 + `dingtalk-connector` 插件** 方案。
...
- 你已经安装并初始化 OpenClaw
- Gateway 可以正常运行
- 星枢 已能读写 `~/.openclaw/openclaw.json`
- 你拥有钉钉企业内部应用的创建和发布权限
...
从当前版本开始，星枢 在打开钉钉配置弹窗时会自动读取 `openclaw.json` 中的 `gateway.auth`：
...
如果你不通过 星枢，而是手改 `~/.openclaw/openclaw.json`，最小示例如下：

...
<p align="center">
  内置 AI 助手?OpenClaw 管理面板 ?一键安装、配置、诊断、修?</p>

<p align="center">
  <strong>🇨🇳 中文</strong> | <a href="README.en.md">🇺🇸 English</a> | <a href="README.zh-TW.md">🇹🇼 繁體中文</a> | <a href="README.ja.md">🇯🇵 日本?/a> | <a href="README.ko.md">🇰🇷 한국?/a> | <a href="README.vi.md">🇻🇳 Tiếng Việt</a> | <a href="README.es.md">🇪🇸 Español</a> | <a href="README.pt.md">🇧🇷 Português</a> | <a href="README.ru.md">🇷🇺 Русский</a> | <a href="README.fr.md">🇫🇷 Français</a> | <a href="README.de.md">🇩🇪 Deutsch</a>
</p>
...
<p align="center">
  <a href="https://gpt.qt.cool"><img src="https://img.shields.io/badge/🔑 晴辰?AI-gpt.qt.cool-6366f1?style=for-the-badge" alt="晴辰?AI"></a>
</p>

- **签到领测试额?* ?每日签到 + 邀请好友，持续获取测试额度
- **兼容 OpenAI 接口** ?无缝对接 OpenClaw，即开即用
- **资源策略** ?限?+ 请求上限，高峰期可能排队
- **模型可用?* ?模型/接口以实际页面为准，可能灰度或版本切?
...
<p align="center">
  ...
  &nbsp;·&nbsp;
  <a href="https://yb.tencent.com/gp/i/IIGXzcMcdh84"><strong>元宝?/strong></a>
  &nbsp;·&nbsp;
...
部署完成后访?`http://服务器IP:1420`，功能与桌面版一致?
📖 详细教程?[Linux 部署指南](docs/linux-deploy.md)
...
📖 详细教程?[Docker 部署指南](docs/docker-deploy.md)（含 Compose、自定义镜像、Nginx 反向代理等）
...
```bash
cd /opt/clawpanel  # 替换为实际安装目?git pull origin main
npm install
npm run build
...
```bash
cd /path/to/clawpanel  # 包含 docker-compose.yml 的目?docker compose build --no-cache clawpanel
...
# 重新启动新容?docker run -d --name clawpanel --restart unless-stopped \
  -p 1420:1420 -v clawpanel-data:/root/.openclaw \
  node:22-slim \
  sh -c "apt-get update && apt-get install -y git && \
    npm install -g @qingchencloud/openclaw-zh --registry https://registry.npmmirror.com && \
...
### macOS / Windows Web 版升?
如果你使用源码部署了 Web 版（非桌面客户端），升级方式?Linux Web 版一致：

```bash
cd clawpanel  # 替换为实际安装目?git pull origin main
npm install
npm run build
npm run serve  # 重新启动
```
...
### Linux Web 版卸?
```bash
# 1. 停止并禁用服?sudo systemctl stop clawpanel
...
> ⚠️ ?5 步会删除所?OpenClaw 配置（API Key、模型设置、Agent 记忆等），请确认不再需要后再执行?
### Docker 卸载

```bash
# 1. 停止并删除容?docker stop clawpanel
docker rm clawpanel

# 2. 删除镜像（可选）
docker rmi $(docker images --filter "reference=*clawpanel*" -q) 2>/dev/null

# 3. 删除数据卷（可选，删除后所有配置将丢失?docker volume rm clawpanel-data
```

如果使用 Docker Compose?
```bash
cd /path/to/clawpanel
docker compose down          # 停止并删除容?docker compose down -v       # 同时删除数据卷（谨慎！）
docker rmi $(docker images --filter "reference=*clawpanel*" -q) 2>/dev/null
```
...
- **🤖 AI 助手（全新·重磅）** ?内置独立 AI 助手? 种操作模?+ 8 大工?+ 交互式问答，详见下方 [AI 助手亮点](#-ai-助手亮点)
- **🖼?图片识别** ?粘贴截图或拖拽图片，AI 自动识别分析，支持多模态图文混排对?- **仪表?* ?系统概览，服务状态实时监控，快捷操作
- **服务管理** ?OpenClaw 启停控制、版本检测与一键升级、Gateway 安装/卸载、配置备份与还原
- **模型配置** ?多服务商管理、模型增删改查、批量连通性测试、延迟检测、拖拽排序、自动保?撤销
- **网关配置** ?端口、访问权限（本机/局域网）、认?Token、Tailscale 组网
- **消息渠道** ?统一管理 Telegram、Discord、飞书、钉钉、QQ 等消息接入，支持同平台多 Agent 绑定
- **通信与自动化** ?消息设置、广播策略、斜杠命令、Webhook、执行审批转发等高级配置
- **使用情况** ?Token 用量、API 费用、热门模?服务?工具排行、每日用量图?- **Agent 管理** ?Agent 增删改查、身份编辑、模型配置、工作区管理
- **聊天** ?流式响应、Markdown 渲染、会话管理?fast /think /verbose /reasoning 命令、Compaction 状态指?- **定时任务** ?Cron 定时执行，支持多渠道投?- **日志查看** ?多日志源实时查看与关键词搜索
- **记忆管理** ?记忆文件查看/编辑、分类管理、ZIP 导出、Agent 切换
- **晴辰?AI 接口** ?内部技术测试平台，兼容 OpenAI 接口，签到领测试额度
- **扩展工具** ?cftunnel 内网穿透管理、ClawApp 状态监?- **关于** ?版本信息、社群入口、相关项目链接、一键升?
## 功能截图
...
<p align="center">
  <img src="docs/00.png" width="800" alt="仪表?>
</p>
<p align="center"><em>仪表??Gateway / 隧道 / 服务实时状态，版本信息、Agent 数量、模型池一屏掌?/em></p>

<p align="center">
  <img src="docs/02.png" width="800" alt="AI 助手设置 ?公益 AI 接口">
</p>
<p align="center"><em>⚙️ AI 设置 ?独立模型配置 + 公益 AI 接口一键接入，GPT-5 全系列免费可?/em></p>

<p align="center">
  <img src="docs/05.png" width="800" alt="AI 助手人设 ?Agent 灵魂">
</p>
<p align="center"><em>?借尸还魂 ??OpenClaw Agent 加载灵魂（SOUL / IDENTITY / USER / AGENTS / TOOLS），继承人格与记?/em></p>

<p align="center">
  <img src="docs/07.png" width="800" alt="实时聊天">
</p>
<p align="center"><em>实时聊天 ?WebSocket 流式对话，多 Provider 模型自动聚合，支持多模?/em></p>

<p align="center">
  <img src="docs/09.png" width="800" alt="模型配置">
</p>
<p align="center"><em>模型配置 ?多服务商统一管理，公益接口一键添加全部模型，主模?备选自动切?/em></p>

<p align="center">
  <img src="docs/13.png" width="800" alt="记忆文件">
</p>
<p align="center"><em>记忆文件 ?工作记忆、记忆归档、核心文件在线编辑，?Agent 记忆隔离</em></p>
...
<p align="center"><em>Agent 管理 ??Agent 创建、身份配置与独立工作区管?/em></p>

<p align="center">
  <img src="docs/11.png" width="800" alt="Gateway 安全认证">
</p>
<p align="center"><em>Gateway ?Token / 密码双认证，Agent 工具权限三档管控，会话可见性控?/em></p>

<p align="center">
  <img src="docs/03.png" width="800" alt="服务管理">
</p>
<p align="center"><em>服务管理 ?启停控制、版本检测、一键升级、npm 源切换、配置备?/em></p>

<p align="center">
  <img src="docs/12.png" width="800" alt="安全设置">
</p>
<p align="center"><em>安全设置 ?访问密码保护与无视风险模?/em></p>

<p align="center">
  <img src="docs/14.png" width="800" alt="扩展工具">
</p>
<p align="center"><em>扩展工具 ?cftunnel 内网穿透、ClawApp 移动客户端一键安?/em></p>

<p align="center">
  <img src="docs/15.png" width="800" alt="系统诊断">
</p>
<p align="center"><em>系统诊断 ?全面健康检测、WebSocket 测试、一键修复配?/em></p>

<p align="center">
  <img src="docs/16.png" width="800" alt="关于">
</p>
<p align="center"><em>关于 ?版本信息、社群入口（QQ / 微信 / 抖音）、相关项目链?/em></p>
...
设置中还?*工具开?*（终?文件），优先级高于模式——关掉终端，即使无限模式也调不了命令?
### 八大工具

| 工具 | 功能 | 示例 |
|------|------|------|
| `ask_user` | 向用户提问（单?多?文本?| "选择要提交到哪个仓库? |
| `get_system_info` | 获取 OS、架构、主目录 | 自动判断该用 PowerShell 还是 Bash |
| `run_command` | 执行 Shell 命令 | 重启 Gateway、查看日?|
| `read_file` | 读取文件 | 读取 openclaw.json 分析配置 |
| `write_file` | 写入文件 | 修复配置错误、生成脚?|
| `list_directory` | 浏览目录 | 列出 .openclaw/ 结构 |
| `list_processes` | 查看进程 | 检?Gateway 是否在运?|
| `check_port` | 检测端口占?| 18789 端口被谁占了?|

### 交互式问答（ask_user?
AI 可以通过 `ask_user` 工具向你提问，支持三种交互方式：

- **单?* ?从多个方案中选一个，还能输入自定义答?- **多?* ?勾选多项，比如"选择要检查的组件"
- **文本** ?自由输入，比?描述你遇到的问题"

AI 等你回答后才会继续操作，实现真正?*人机协作**?
### PR 助手 & Bug 报告

发现 Bug？AI 不只是告诉你怎么修——它**直接帮你?*?
1. 🐛 **提交 Bug 报告** ?AI 自动收集系统环境、读取错误日志，按标准模板整理成 GitHub Issue，你复制粘贴就能提交
2. 🔀 **PR 助手** ?AI 分析 Bug 根因 ?定位代码 ?生成修复方案 ?通过 `run_command` 执行 git 命令完成 Fork/Branch/Commit/Push?*用户只需点确?*
...
| 技?| 功能 |
|------|------|
| 🔧 检查配?| 读取并分?openclaw.json |
| 🏥 诊断 Gateway | 检查进程、端口、日?|
| 📂 浏览目录 | 查看 .openclaw 目录结构 |
| 💻 检查环?| Node.js、npm 版本检?|
| 📋 分析日志 | 搜索 ERROR/WARN 关键?|
| 🔨 一键排?| 自动检测并修复常见问题 |
| 🐛 提交 Bug | 整理 Issue 提交?GitHub |
| 🔀 PR 助手 | 定位 Bug 并生成修?PR |

## 技术架?
| 层级 | 技?| 说明 |
|------|------|------|
| 前端 | Vanilla JS + Vite | 零框架依赖，轻量快?|
| 后端 | Rust + Tauri v2 | 原生性能，跨平台打包 |
| 通信 | Tauri IPC + Shell Plugin | 前后端桥接，本地命令执行 |
| 样式 | ?CSS（CSS Variables?| 暗色/亮色主题，玻璃拟态风?|

```
clawpanel/
├── src/                    # 前端源码
?  ├── pages/              # 10 个页面模??  ├── components/         # 通用组件（侧边栏、弹窗、Toast??  ├── lib/                # 工具库（Tauri API 封装、主题）
?  ├── style/              # 样式文件
?  ├── router.js           # 路由
?  └── main.js             # 入口
├── src-tauri/              # Rust 后端
...
├── public/                 # 静态资?├── scripts/                # 开发与构建脚本
?  ├── dev.sh              # 开发模式启??  └── build.sh            # 编译与打?├── .github/workflows/      # CI/CD
?  ├── ci.yml              # 持续集成（push/PR 自动检查）
?  └── release.yml         # 发布构建（全平台打包?├── index.html              # HTML 入口
...
- Tauri v2 系统依赖（参?[Tauri 官方文档](https://v2.tauri.app/start/prerequisites/)?
### 安装与开?
```bash
...
# 仅启?Vite 前端（浏览器调试，使?mock 数据?./scripts/dev.sh web
```
...
# 仅启?Vite 前端（浏览器调试，使?mock 数据?npm run dev
```
...
# 编译正式发布版本（含打包?./scripts/build.sh release
```
...
如果你只想开发前端或部署 Web 版，**不需要安?Rust**?
```bash
# 克隆并安?git clone https://github.com/qingchencloud/clawpanel.git
cd clawpanel
npm install

# 开发模式（热更新，自带 API mock 后端?npm run dev
# 浏览器打开 http://localhost:1420

# 构建生产版本
npm run build

# 启动 Web 服务器（Headless，适用?Linux/ARM/Docker?npm run serve
# 默认监听 0.0.0.0:1420，支?--port ?--host 参数
```

Web 版功能与桌面版一致，后端通过 `scripts/dev-api.js` 调用本机 OpenClaw CLI 实现?
> **ARM/Armbian 用户**：Web 模式天然兼容 ARM64 设备，详?[Armbian 部署指南](docs/armbian-deploy.md)?
## 快速上?
安装完成后，按以下步骤即可开始使用：
...
| Ollama（本地） | 免费，无需 Key，安装后自动检?|

填入 `Base URL` ?`API Key`，点击「测试连接」确认可用，然后保存?
### 3. 启动 Gateway

前往**服务管理**页面，点击「启动」按钮启?Gateway。状态变为绿色即可?
> Gateway ?OpenClaw 的核心服务，负责处理 AI 对话请求?
### 4. 开始聊?
...
部署完成后访?`http://服务器IP:1420`?
### 安全注意事项

> ⚠️ **公网暴露风险**：Web 版默认监听所有网卡。请务必?
1. **设置访问密码** ?首次登录会提示修改默认密码，或在「安全设置」页面配?2. **防火墙限?* ?仅开?1420 端口给可?IP
...
- API Key 存储在服务器?`~/.openclaw/openclaw.json` 中，**不会**传输到外?- 建议?`~/.openclaw/` 目录设置 `chmod 700` 权限
...
| 平台 | 类型 | 配置难度 | 说明 |
|------|------|---------|------|
| 飞书 / Lark | 企业内部应用 | ⭐⭐ | 支持内置插件和官方插件两种模?|
| 钉钉 | 企业内部应用 | ⭐⭐ | Stream 模式，需创建机器人应?|
| Telegram | Bot | ?| 通过 @BotFather 创建 Bot |
| Discord | Bot | ⭐⭐ | 需创建 Application + Bot |
| QQ 机器?| 官方 Bot | ⭐⭐?| 需?QQ 开放平台注?|
...
- 消息渠道需?**Gateway 正在运行**才能接收消息
- 每个平台需?*配对审批**才能连接（在渠道设置中完成）
...
### Windows 安装?exit 128 (access rights)

npm 依赖需?Git。如果已?Git 但仍?128，是因为依赖用了 SSH 协议拉代码但你没?GitHub SSH Key。运行以下命令改?HTTPS?
```powershell
git config --global url."https://github.com/".insteadOf ssh://git@github.com/
git config --global url."https://github.com/".insteadOf git@github.com:
```

没装 Git 的请先安?[Git for Windows](https://git-scm.com/download/win)?*v0.4.2+ 已自动配?HTTPS 模式?*
...
| 症状 | 原因 | 解决 |
|------|------|------|
| 端口 18789 被占?| 另一?Gateway 进程残留 | 终端执行 `pkill -f openclaw` 后重?|
| 配置文件损坏 | openclaw.json 格式错误 | 前往「服务管理」→「从备份恢复?|
| 反复崩溃 | API Key 或模型配置异?| ?AI 助手「?一键排障」自动诊?|
| 提示 "auth mode" 错误 | 认证配置不兼?| 在「安全设置」重?Gateway 认证 |
...
升级?**v0.8.5+**。旧版本?Web 后端缺少部分命令实现，v0.8.5 已补全所?handler?
```bash
cd /opt/clawpanel  # 替换为实际安装目?git pull origin main
npm install
npm run build
sudo systemctl restart clawpanel  # ?pm2 restart clawpanel
```

### 消息渠道保存后不生效

1. 确认 Gateway 正在运行
2. 飞书/钉钉：需要在对应开放平?*发布应用版本**
3. 飞书：私聊测试需在「工作台」搜索机器人名称；群聊需通过「群设置 ?智能群助手」添?4. 钉钉：消息接收模式必须选择 **Stream 模式**
  ...
  <tr>
    <td align="center">
      <a href="https://www.ciyundata.com/cart/goods.htm?id=267" target="_blank">
        <img src="https://img.shields.io/badge/慈云数据-香港直连2H2G_99??FF6B35?style=for-the-badge&logo=icloud&logoColor=white" alt="慈云数据">
      </a>
      <br><br>
      <sub>慈云数据服务团队?2020 年成立，专注于基础云计算、游戏云、主题站长等业务</sub>
      <br>

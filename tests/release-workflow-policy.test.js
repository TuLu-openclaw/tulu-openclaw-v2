...
const engineManagerSource = readFileSync(new URL('../src/lib/engine-manager.js', import.meta.url), 'utf8')
const hermesEngineSource = readFileSync(new URL('../src/engines/hermes/index.js', import.meta.url), 'utf8')
const hermesChatSource = readFileSync(new URL('../src/engines/hermes/pages/chat.js', import.meta.url), 'utf8')
const hermesMemorySource = readFileSync(new URL('../src/engines/hermes/pages/memory.js', import.meta.url), 'utf8')
const hermesSetupSource = readFileSync(new URL('../src/engines/hermes/pages/setup.js', import.meta.url), 'utf8')
...
test('engine switching has a single teardown contract and cannot strand Hermes UI', () => {
  assert.ok(routerSource.includes('page.__cleanup'))
  assert.ok(routerSource.includes('elementCleanup'))
  assert.ok(engineManagerSource.includes('_switchPromise'))
  assert.ok(engineManagerSource.includes('引擎启动超时'))
  assert.ok(engineManagerSource.includes('通知监听器 once'))
  assert.ok(hermesEngineSource.includes('_bootGeneration'))
  assert.ok(hermesEngineSource.includes('if (generation !== _bootGeneration) return'))
  assert.ok(hermesChatSource.includes('registerCleanup?.(() => close(null))'))
  assert.ok(hermesChatSource.includes('el.__cleanup = () =>'))
  assert.ok(hermesMemorySource.includes('activeOverlay?.remove()'))
  assert.ok(hermesMemorySource.includes('el.__cleanup = () =>'))
  assert.ok(hermesSetupSource.includes('removeEventListener(\'click\', onDocumentClick)'))
})

test('installation verifies the new CLI before preserving shared npm shims', () => {
  const verifyIndex = rustConfig.indexOf('let verified_cli =')
  const preserveIndex = rustConfig.indexOf('旧来源包已保留为回退副本')
  assert.ok(verifyIndex >= 0)
  assert.ok(preserveIndex > verifyIndex)
  assert.ok(rustConfig.includes('已保留原有安装，未清理旧版本'))
  assert.ok(rustConfig.includes('安装后的 OpenClaw 来源未切换成功'))
  assert.ok(!rustConfig.includes('清理旧版本 ({old_pkg})'))
})

...
test('browser-use install is pinned and isolated', () => {
  ...
  assert.match(backend, /run_checked\(\s*&venv_python\(\),\s*&\[\s*"-m"\.into\(\),\s*"playwright"\.into\(\),\s*"install"\.into\(\),\s*"chromium"\.into\(\)/s)
  assert.doesNotMatch(backend, /"tool"\.into\(\),\s*"run"\.into\(\),\s*"playwright"\.into\(\)/s)
  assert.match(backend, /super::apply_proxy_env\(&mut command\)/)
  assert.match(backend, /super::build_http_client\(Duration::from_secs\(120\)/)
  assert.match(backend, /fn uv_expected_sha256\(\)/)
  assert.match(backend, /Sha256::digest\(&bytes\)/)
  assert.match(backend, /uv 运行时完整性校验失败/)
  assert.match(api, /browserUseInstall:[^\n]*1900000/)

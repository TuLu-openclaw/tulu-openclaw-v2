...
let DEVICE_KEY_FILE = path.join(OPENCLAW_DIR, '星枢OpenClaw-device-key.json')
...
const CLUSTER_TOKEN = '星枢OpenClaw-cluster-secret-2026'
const PANEL_CONFIG_PATH = path.join(DEFAULT_OPENCLAW_DIR, '星枢OpenClaw.json')
...
function applyOpenclawPathConfig(panelConfig) {
  ...
  DEVICE_KEY_FILE = path.join(OPENCLAW_DIR, '星枢OpenClaw-device-key.json')
...
function stripUiFields(config) {
  ...
  // 清理根层级 星枢OpenClaw 内部字段（version info 等），避免污染 openclaw.json
...
function patchGatewayOrigins() {
  ...
  // 合并：保留用户已有的 origins，只追加 星枢OpenClaw 需要的
...
function winStartGateway() {
  ...
  fs.appendFileSync(logPath, `\n[${timestamp}] [星枢OpenClaw] Starting Gateway on Windows...\n`)
...
function matchesCurrentGatewayOwnerSignature(owner) {
  if (!owner || owner.startedBy !== '星枢OpenClaw') return false
...
function linuxStartGateway() {
  ...
  const timestamp = new Date().toISOString()
  fs.appendFileSync(logPath, `\n[${timestamp}] [星枢OpenClaw] Starting Gateway on Linux...\n`)
...
// 查找 星枢OpenClaw-agent.cjs 脚本并注入到容器（.cjs 避免容器内 ESM 冲突）
function findAgentScript() {
  const candidates = [
    path.resolve(__dev_dirname, '../openclaw-docker/full/星枢OpenClaw-agent.cjs'),
    path.resolve(__dev_dirname, '../openclaw-docker/full/星枢OpenClaw-agent.js'),
    path.resolve(__dev_dirname, '../../openclaw-docker/full/星枢OpenClaw-agent.cjs'),
    path.resolve(__dev_dirname, '../../openclaw-docker/full/星枢OpenClaw-agent.js'),
    path.resolve(__dev_dirname, '../星枢OpenClaw-agent.cjs'),
    path.resolve(__dev_dirname, '../星枢OpenClaw-agent.js'),
    path.resolve(__dev_dirname, '星枢OpenClaw-agent.cjs'),
    path.resolve(__dev_dirname, '星枢OpenClaw-agent.js'),
  ]
  for (const p of candidates) {
...
async function injectAgentToContainer(containerId, endpoint, cExecFn, agentScript = null) {
  ...
  if (!source) {
    console.warn('[agent] 星枢OpenClaw-agent.cjs 未找到，跳过注入')
    return false
  }
  const b64 = Buffer.from(source.content, 'utf8').toString('base64')
  await cExecFn(`echo '${b64}' | base64 -d > /app/星枢OpenClaw-agent.cjs`)
...
function _normalizeAssistantApiType(apiType = 'openai-completions') {
  return ['anthropic', 'anthropic-messages'].includes(apiType) ? 'anthropic-messages'
    : apiType === 'google-gemini' ? 'google-gemini'
    : apiType === 'openai-responses' ? 'openai-responses'
    : 'openai-completions'
}

function _extractProviderError(text, status) {
  let msg = `HTTP ${status}`
  try {
    ...
    else if (parsed.detail) msg = typeof parsed.detail === 'string' ? parsed.detail : JSON.stringify(parsed.detail)
  } catch {
...
function _extractAssistantReply(data) {
...
const handlers = {
  ...
  async docker_agent({ nodeId, containerId, cmd } = {}) {
    ...
    const runAgent = async () => {
      const execResult = await dockerExecRun(
        containerId,
        ['node', '/app/星枢OpenClaw-agent.cjs', cmdJson],
        node.endpoint,
        timeout,
      )
      return execResult
  ...
  async docker_init_worker({ nodeId, containerId, role = 'general' } = {}) {
    ...
    try {
      ...
      const ROLE_AGENTS = {
        general: '# 操作指令\n\n你是龙虾军团的步兵，接受统帅通过 星枢OpenClaw 下达的任务指令。\n\n## 规则\n- 收到任务后立即执行，完成后简要汇报结果\n- 如果任务不清楚，先确认再行动\n- 保持回复简洁，重点突出\n- 你有独立的记忆空间，会自动记录重要信息',
        coder: '# 操作指令\n\n你是龙虾军团的突击兵，专精编程作战。\n\n## 规则\n- 收到编程任务后，先分析需求再写代码\n- 代码必须可运行，包含必要的注释\n- 主动进行错误处理和边界检查\n- 如果涉及多个文件，说明修改顺序\n- 完成后给出测试建议\n\n## 专长\n- 全栈开发、API 设计、数据库优化\n- Bug 定位与修复、代码重构\n- 性能优化、安全审计',
        translator: '# 操作指令\n\n你是龙虾军团的翻译官，专精多语言互译。\n\n## 规则\n- 翻译要信达雅，保持原文风格\n- 专业术语保留原文标注\n- 长文分段翻译，保持上下文一致\n- 文学作品注重意境传达\n- 技术文档注重准确性\n\n## 专长\n- 中英日韩法德西等主流语言\n- 技术文档、文学作品、商务邮件',
        writer: '# 操作指令\n\n你是龙虾军团的文书官，专精写作任务。\n\n## 规则\n- 根据场景调整语气和风格\n- 注重结构清晰、逻辑连贯\n- 创意写作要有个性和亮点\n- 技术文档要准确严谨\n- 营销文案要抓住痛点\n\n## 专长\n- 博客文章、技术文档、营销文案\n- 故事创作、剧本、诗歌\n- SEO 优化、社交媒体内容',
    ...
    // 4.5 注入 星枢OpenClaw Agent（容器内专属控制代理）
    try {
      await injectAgentToContainer(containerId, node.endpoint, cExec)
      results.files.push('星枢OpenClaw-agent.cjs')
    } catch (e) {
  ...
  // 部署模式检测
  get_deploy_mode() {
    const inDocker = fs.existsSync('/.dockerenv') || (process.env.星枢OpenClaw_MODE === 'docker')
  ...
  async test_model({ baseUrl, apiKey, modelId, apiType = 'openai-completions' }) {
    const type = _normalizeAssistantApiType(apiType)
    ...
    try {
      ...
      if (type === 'anthropic-messages') {
        ...
        resp = await _fetchJsonWithTimeout(`${base}/messages`, {
          method: 'POST',
          headers,
          body: JSON.stringify({
            model: modelId,
        ...
        resp = await _fetchJsonWithTimeout(`${base}/models/${encodeURIComponent(modelId)}:generateContent?key=${encodeURIComponent(apiKey || '')}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        resp = await _fetchJsonWithTimeout(`${base}/responses`, {
          method: 'POST',
          headers,
        ...
        const headers = { 'Content-Type': 'application/json' }
        if (apiKey) headers.Authorization = `Bearer ${apiKey}`
        resp = await _fetchJsonWithTimeout(`${base}/chat/completions`, {
          method: 'POST',
          headers,
          body: JSON.stringify({
            model: modelId,
      ...
      if (!resp.ok) {
        const text = await resp.text()
        const msg = _extractProviderError(text, resp.status)
        if (resp.status === 401 || resp.status === 403) throw new Error(msg)
        return `⚠ 连接正常（API 返回 ${resp.status}，部分模型对简单测试不兼容，不影响实际使用）`
      }
      const data = await resp.json()
      return _extractAssistantReply(data) || '（无回复内容）'
    } catch (e) {
      throw e
    }
  },

  async assistant_chat_once({ baseUrl, apiKey, modelId, apiType = 'openai-completions', messages = [], temperature = 0.7 }) {
    const type = _normalizeAssistantApiType(apiType)
    let base = _normalizeBaseUrl(baseUrl)
    if (type === 'anthropic-messages' && !/\/v1$/i.test(base)) base += '/v1'
    ...
    if (type === 'anthropic-messages') {
      ...
      resp = await _fetchJsonWithTimeout(`${base}/messages`, { method: 'POST', headers, body: JSON.stringify(body) }, 120000)
    } else if (type === 'google-gemini') {
      const contents = msgs
      ...
      const headers = { 'Content-Type': 'application/json' }
      if (apiKey) headers.Authorization = `Bearer ${apiKey}`
      const input = msgs.map(m => `${m.role || 'user'}: ${typeof m.content === 'string' ? m.content : JSON.stringify(m.content ?? '')}`).join('\n\n')
      resp = await _fetchJsonWithTimeout(`${base}/responses`, {
        method: 'POST',
        headers,
        body: JSON.stringify({ model: modelId, input, temperature, max_output_tokens: 8192 }),
      }, 120000)
    } else {
      const headers = { 'Content-Type': 'application/json' }
      if (apiKey) headers.Authorization = `Bearer ${apiKey}`
      resp = await _fetchJsonWithTimeout(`${base}/chat/completions`, {
        method: 'POST',
        headers,
        body: JSON.stringify({ model: modelId, messages: msgs, stream: false, temperature }),
      }, 120000)
    }
    const text = await resp.text()
    if (!resp.ok) throw new Error(_extractProviderError(text, resp.status))
    let data
    try { data = JSON.parse(text) } catch (e) { throw new Error(`解析响应失败: ${e.message}`) }
    const reply = _extractAssistantReply(data)
    if (!reply) throw new Error('模型响应为空或格式不支持')
    return reply
  },
  ...
  create_connect_frame({ nonce, gatewayToken }) {
    ...
    return {
      ...
      params: {
        ...
        locale: 'zh-CN', userAgent: '星枢OpenClaw/1.0.0 (web)',
      },
    }
  },
  // 数据目录 & 图片存储
  assistant_ensure_data_dir() {
    const dataDir = path.join(OPENCLAW_DIR, '星枢OpenClaw')
  ...
  assistant_save_image({ id, data }) {
    const dir = path.join(OPENCLAW_DIR, '星枢OpenClaw', 'images')
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
  ...
  get_update_status() {
    const pkgPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'package.json')
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'))
    return { currentVersion: pkg.version, updateReady: false, updateVersion: '', updateDir: path.join(OPENCLAW_DIR, '星枢OpenClaw', 'web-update') }
  },

...
let DEVICE_KEY_FILE = path.join(OPENCLAW_DIR, '星枢-device-key.json')
...
const CLUSTER_TOKEN = '星枢-cluster-secret-2026'
const PANEL_CONFIG_PATH = path.join(DEFAULT_OPENCLAW_DIR, '星枢.json')
...
function applyOpenclawPathConfig(panelConfig) {
  ...
  DEVICE_KEY_FILE = path.join(OPENCLAW_DIR, '星枢-device-key.json')
...
function stripUiFields(config) {
  ...
  // 清理根层级 星枢 内部字段（version info 等），避免污染 openclaw.json
...
function winStartGateway() {
  ...
  fs.appendFileSync(logPath, `\n[${timestamp}] [星枢] Starting Gateway on Windows...\n`)
...
function matchesCurrentGatewayOwnerSignature(owner) {
  if (!owner || owner.startedBy !== '星枢') return false
...
function linuxStartGateway() {
  ...
  const timestamp = new Date().toISOString()
  fs.appendFileSync(logPath, `\n[${timestamp}] [星枢] Starting Gateway on Linux...\n`)
...
// 查找 星枢-agent.cjs 脚本并注入到容器（.cjs 避免容器内 ESM 冲突）
function findAgentScript() {
  const candidates = [
    path.resolve(__dev_dirname, '../openclaw-docker/full/星枢-agent.cjs'),
    path.resolve(__dev_dirname, '../openclaw-docker/full/星枢-agent.js'),
    path.resolve(__dev_dirname, '../../openclaw-docker/full/星枢-agent.cjs'),
    path.resolve(__dev_dirname, '../../openclaw-docker/full/星枢-agent.js'),
    path.resolve(__dev_dirname, '../星枢-agent.cjs'),
    path.resolve(__dev_dirname, '../星枢-agent.js'),
    path.resolve(__dev_dirname, '星枢-agent.cjs'),
    path.resolve(__dev_dirname, '星枢-agent.js'),
  ]
  for (const p of candidates) {
...
async function injectAgentToContainer(containerId, endpoint, cExecFn, agentScript = null) {
  ...
  if (!source) {
    console.warn('[agent] 星枢-agent.cjs 未找到，跳过注入')
    return false
  }
  const b64 = Buffer.from(source.content, 'utf8').toString('base64')
  await cExecFn(`echo '${b64}' | base64 -d > /app/星枢-agent.cjs`)
...
const handlers = {
  ...
  // 部署模式检测
  get_deploy_mode() {
    const inDocker = fs.existsSync('/.dockerenv') || (process.env.星枢_MODE === 'docker')
  ...
  async upgrade_openclaw({ source = 'chinese', version, method = 'auto' } = {}) {
    ...
    if (!version && recommended) {
      logs.push(`星枢 ${PANEL_VERSION} 默认绑定 OpenClaw 稳定版: ${recommended}`)
    }
  ...
  create_connect_frame({ nonce, gatewayToken }) {
    ...
    return {
      ...
      params: {
        ...
        locale: 'zh-CN', userAgent: '星枢/1.0.0 (web)',
      },
    }
  },
  // 数据目录 & 图片存储
  assistant_ensure_data_dir() {
    const dataDir = path.join(OPENCLAW_DIR, '星枢')
  ...
  assistant_save_image({ id, data }) {
    const dir = path.join(OPENCLAW_DIR, '星枢', 'images')
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
  ...
  check_panel_update() { return { latest: null, url: 'https://github.com/qingchencloud/星枢/releases' } },
  ...
  download_frontend_update() { return { success: true, files: 12, path: path.join(OPENCLAW_DIR, '星枢', 'web-update') } },
  rollback_frontend_update() { return { success: true } },
  get_update_status() {
    const pkgPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'package.json')
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'))
    return { currentVersion: pkg.version, updateReady: false, updateVersion: '', updateDir: path.join(OPENCLAW_DIR, '星枢', 'web-update') }
  },

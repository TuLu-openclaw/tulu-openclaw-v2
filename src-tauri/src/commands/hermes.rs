...
fn hermes_direct_custom_openai_config() -> Option<(String, String, String)> {
    let home = hermes_home();
    let config_raw = std::fs::read_to_string(home.join("config.yaml")).ok()?;
    let mut in_model = false;
    ...
    for line in config_raw.lines() {
        ...
        if in_model {
            ...
            if let Some(v) = trimmed.strip_prefix("default:") {
                ...
                provider = v.trim().trim_matches('"').to_string();
            } else if let Some(v) = trimmed.strip_prefix("base_url:") {
                base_url = v.trim().trim_matches('"').trim_end_matches('/').to_string();
            }
        }
    }

    if provider != "custom" || model.trim().is_empty() || base_url.trim().is_empty() {
        return None;
    }

    let env_raw = std::fs::read_to_string(home.join(".env")).ok()?;
    ...
    for line in env_raw.lines() {
        ...
        if let Some(v) = trimmed.strip_prefix("CUSTOM_API_KEY=") {
            api_key = v.trim().to_string();
            break;
        }
        if api_key.is_empty() {
            if let Some(v) = trimmed.strip_prefix("OPENAI_API_KEY=") {
                api_key = v.trim().to_string();
            }
        }
    }
...
async fn hermes_agent_run_direct_custom_openai(
    app: tauri::AppHandle,
    input: String,
    ...
    let run_id = format!("direct-{}", chrono::Utc::now().timestamp_millis());
    let _ = app.emit(
        "hermes-run-started",
    ...
    if let Some(inst) = instructions.as_ref().filter(|s| !s.trim().is_empty()) {
        messages.push(serde_json::json!({ "role": "system", "content": inst }));
    }
    if let Some(hist) = conversation_history.as_ref().and_then(|v| v.as_array()) {
        for msg in hist {
    ...
    let client = super::build_http_client_no_proxy(std::time::Duration::from_secs(120), None)
        .map_err(|e| format!("HTTP 客户端创建失败: {e}"))?;
    let resp = client
        .post(&url)
        .header("Authorization", format!("Bearer {api_key}"))
    ...
    let value: Value = serde_json::from_str(&text).map_err(|e| format!("解析模型响应失败: {e}"))?;
    let output = value
        .get("choices")
        .and_then(|c| c.get(0))
    ...
    let _ = app.emit(
        "hermes-run-delta",
        serde_json::json!({ "run_id": &run_id, "delta": &output }),
    );
    let _ = app.emit(
        "hermes-run-done",
        serde_json::json!({ "run_id": &run_id, "output": &output }),
    );
    let _ = session_id;
    Ok(run_id)
}
...
pub async fn hermes_agent_run(
    ...
    // Hermes Gateway 的 custom provider 在部分 OpenAI-compatible 聚合接口上会误走
    // 自身 SDK/agent-loop 路径并返回 401，但同一 base_url/api_key/model 使用标准
    // /chat/completions 可正常工作。对 ClawPanel 导入的 custom 配置，先走
    // Tauri 后端直连兜底，避免用户明明语言模型可用却卡死在 INVALID_API_KEY。
    if let Some((base_url, api_key, model)) = hermes_direct_custom_openai_config() {
        return hermes_agent_run_direct_custom_openai(
            app,
            input,
            session_id,

...
# Find the current open_global_builtin_window function
pattern = r'(/// 全球内置窗口：iframe 加载外部 URL \+ 密码验证 \+ 悬浮按钮.*?\n)(?:.*?\n)*?(\n    \}\n)'
match = re.search(pattern, c, re.DOTALL)
if not match:
    print('Could not find function, trying alternate pattern')
    # Try to find by function signature
    pattern2 = r'(/// 全球内置窗口.*?(?:\n    ///.*?)*?\n)(?:pub async fn open_global_builtin_window.*?\n)(?:.*?\n)*?(\n    \}\n)'
    match = re.search(pattern2, c, re.DOTALL)
    if not match:
        print('Still could not find function')
        # Let's print what's around the function
        idx = c.find('open_global_builtin_window')
        if idx >= 0:
            print(f'Found at idx {idx}')
            print(repr(c[idx-200:idx+500]))
        exit(1)

func_start = match.start(1)
func_end = match.end(2)
...
/// - Overlay 窗口：透明悬浮按钮，通过 Tauri 事件系统与主窗口通信
...
pub async fn open_global_builtin_window(
    ...
    // 通知 overlay 主窗口已创建
    let _ = overlay_window.emit("main_window_ready", serde_json::json!({
        "main_label": label,

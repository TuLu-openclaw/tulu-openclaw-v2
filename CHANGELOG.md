...
- **厂长详情标识闭环** — 统一列表、搜索、片库卡片、追剧和播放历史使用的详情引用，支持数字 ID 与 `/movie/<id>.html`，不再把空值传给详情加载器。
- **旧记录自动恢复** — 旧版本已保存但缺少详情 ID 的厂长历史和追剧记录，可按片名实时搜索恢复详情，不会在界面层提前报“缺少 movie id”。
- **真实媒体地址保真** — 播放包装页提取 `m3u8`/`mp4` 时保留完整查询参数，避免丢失 token 等鉴权字段导致播放失败。
- **失效来源完整退役** — 删除原云岚来源的导航、分类、搜索、列表、详情、图片、历史恢复和后端无效证书兼容分支，不再保留不可验证的运行时入口。
- **失效专题入口清理** — 删除已无实现函数的旧专题入口和分页分支，片库卡片统一进入标准详情与追剧流程。

### 验证 (Verification)

- 厂长列表 → 详情 → 多集 → 播放包装页 → 带鉴权参数的真实 `m3u8` 夹具链路通过
- 厂长普通卡片、搜索、追剧和播放历史重开使用同一详情引用契约
- 全仓扫描确认失效来源无运行时地址、源键、解析器或后端专用网络分支
- `npm test` 235/235 通过
- `npm run build` 通过
- `npm audit --omit=dev` 0 vulnerabilities
- Rust 格式检查通过；完整多平台编译由 GitHub Release 工作流门禁执行
...
- **关于页面公司信息** — 新增「关于我们」板块：武汉星枢OpenClaw天下网络科技有限公司
